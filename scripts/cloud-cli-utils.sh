...
#     fi
#     RESULT=$(aws codebuild start-build-batch --profile="${profile_name}" --project-name $project_name --source-version=$target_branch \
#      --image-override '671107461633.dkr.ecr.us-east-1.amazonaws.com/amplify-cli-codebuild-testing-stack-prod-amplifyclie2eecrc5fd2bba-ke8oxaimjhf2:dotnet8' \
#      --environment-variables-override name=BRANCH_NAME,value=$target_branch,type=PLAINTEXT name=WINDOWS_IMAGE_2019,value=671107461633.dkr.ecr.us-east-1.amazonaws.com/amplify-cli-codebuild-testing-stack-prod-amplifyclie2eecrc5fd2bba-ke8oxaimjhf2:windows2019-1.0.3-1,type=PLAINTEXT $npm_variable_override \
#      --query 'buildBatch.id' --output text)
#     echo "https://us-east-1.console.aws.amazon.com/codesuite/codebuild/$account_number/projects/$project_name/batch/$RESULT?region=us-east-1"
# }
function triggerProjectBatch {
    ...
    RESULT=$(aws codebuild start-build-batch --profile="${profile_name}" --project-name $project_name --source-version=$target_branch \
     ...
     --environment-variables-override name=BRANCH_NAME,value=$target_branch,type=PLAINTEXT name=WINDOWS_IMAGE_2019,value=aws/codebuild/windows-base:2019-3.0,type=PLAINTEXT $npm_variable_override \
     --query 'buildBatch.id' --output text)
    echo "https://us-east-1.console.aws.amazon.com/codesuite/codebuild/$account_number/projects/$project_name/batch/$RESULT?region=us-east-1"
}
