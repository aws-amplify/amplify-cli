...
function cloudGen2MigrationLocal {
    echo "Running Gen2 Migration E2E Tests (Local Account) on branch: $CURR_BRANCH"
    
    account_number=$E2E_ACCOUNT_LOCAL
    role_name=$E2E_ROLE_NAME
    profile_name="${E2E_PROFILE_NAME}Local"
...
# Run a single migration test from source (no pkg binary) - for debugging
function cloudGen2MigrationTest {
    echo "Running Gen2 Migration E2E Test from source on branch: $CURR_BRANCH"
    ...
    echo "AWS Account: $account_number"
    echo "Project: $E2E_PROJECT_NAME"
    echo "Buildspec: codebuild_specs/gen2_migration_from_source_test.yml"
    
    RESULT=$(aws codebuild start-build \
      --profile="${profile_name}" \
      --project-name $E2E_PROJECT_NAME \
      --source-version=$CURR_BRANCH \
      --buildspec-override "codebuild_specs/gen2_migration_from_source_test.yml" \
      --environment-variables-override name=BRANCH_NAME,value=$CURR_BRANCH,type=PLAINTEXT \
      --query 'build.id' --output text)
    
    echo ""
    echo "Build started! View at:"
    echo "https://us-east-1.console.aws.amazon.com/codesuite/codebuild/$account_number/projects/$E2E_PROJECT_NAME/build/$RESULT?region=us-east-1"
}
