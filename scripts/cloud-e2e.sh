...
function triggerBuild {
    ...
    RESULT=$(aws codebuild start-build-batch --profile="${CLOUD_E2E_PROFILE}" --project-name AmplifyCLI-E2E-Testing --source-version=$TARGET_BRANCH --build-timeout-in-minutes-override 300 --git-clone-depth-override=1000 --debug-session-enabled --query 'buildBatch.id' --output text)
