...
function triggerBuild {
    ...
    RESULT=$(aws codebuild start-build-batch --profile="${CLOUD_E2E_PROFILE}" --project-name AmplifyCLI-E2E-Testing --source-version=$TARGET_BRANCH --environment-variables-override name=BRANCH_NAME,value=$TARGET_BRANCH,type=PLAINTEXT --query 'buildBatch.id' --output text --debug-session-enabled --build-timeout-in-minutes-override 360)
