...
export class S3InputState {
  ...
  constructor(private readonly context: $TSContext, resourceName: string, userInput: S3UserInputs | undefined) {
    this._cliInputsFilePath = pathManager.getResourceInputsJsonFilePath(undefined, AmplifyCategories.STORAGE, resourceName);
    this._category = AmplifyCategories.STORAGE;
    this._resourceName = resourceName;
    const resourcePath = pathManager.getResourceDirectoryPath(undefined, AmplifyCategories.STORAGE, resourceName);
    this._service = AmplifySupportedService.S3;
    this.buildFilePath = path.resolve(path.join(resourcePath, 'build'));
    if (userInput) {
      // Add flow
  ...
  public async isCLIInputsValid(cliInputs?: S3UserInputs): Promise<boolean> {
    return this.validateInputs(cliInputs ?? this.getCliInputPayload());
  }
  ...
  private validateInputs = async (inputs?: S3UserInputs): Promise<boolean> => {
    ...
    return schemaValidator.validateInput(JSON.stringify(inputs ?? this._inputPayload));
  }

  updateInputPayload = async (props: S3InputStateOptions) => {
    // Overwrite
