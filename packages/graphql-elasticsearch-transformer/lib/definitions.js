...
function makeSearchableScalarInputObject(type) {
    var name = graphql_transformer_common_1.graphqlName("Searchable" + type + "FilterInput");
...
function makeSearchableXFilterInputObject(obj) {
    var name = graphql_transformer_common_1.graphqlName("Searchable" + obj.name.value + "FilterInput");
    var fields = obj.fields
        .filter(function (field) { return graphql_transformer_common_1.isScalar(field.type) === true; })
        .map(function (field) { return ({
        kind: graphql_1.Kind.INPUT_VALUE_DEFINITION,
        name: field.name,
        type: graphql_transformer_common_1.makeNamedType('Searchable' + graphql_transformer_common_1.getBaseType(field.type) + 'FilterInput'),
        // TODO: Service does not support new style descriptions so wait.
        // description: field.description,
        directives: []
    }); });
