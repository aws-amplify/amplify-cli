...
export class SwiftDeclarationBlock {
    ...
    private generateEnumStr(): string {
        ...
        const enumValues = this.mergeSections(
            Object.entries(this._enumValues).map(([name, val]) =>
                ['case', name, ...(name !== val ? ['=', `"${val}"`] : [])].join(' ')
            ),
            false
        );
        const declarationFoot = '}';
        return this.mergeSections(
            [declarationHead, indentMultiline(enumValues), declarationFoot],
            false
        );
    }
    ...
    private generateStructOrExtensionStr(): string {
        ...
        const declarationBody = indentMultiline(
            this.mergeSections([...this._block, properties, methods])
        );
        const declarationFoot = '}';
        return this.mergeSections(
            [this._comment, declarationHead, declarationBody, declarationFoot],
            false
        );
    }
    ...
    private generateArgsStr(args: MethodArgument[]): string {
        const res: string[] = args.reduce((acc: string[], arg) => {
            const val: string | null = arg.value
                ? arg.value
                : arg.flags.isList
                ? '[]'
                : arg.flags.optional
            ...
            acc.push(
                [
                    ...
                    ': ',
                    type,
                    !arg.flags.isList && arg.flags.optional ? '?' : '',
                    val ? ` = ${val}` : ''
                ].join('')
            );
            return acc;
    ...
    private generatePropertiesStr(prop: StructProperty): string {
        ...
        const propertyType = propertyTypeName
            ? `:${propertyTypeName}${prop.flags.optional && !prop.flags.isList ? '?' : ''}`
            : '';
        ...
        if (setterStr) {
            getterSetterStr = this.mergeSections(
                [
                    '{',
                    indentMultiline(`set: ${setterStr}`),
                    getterStr ? indentMultiline(`get: ${getterStr}`) : '',
                    '}'
                ],
                false
            );
        ...
        return this.mergeSections([
            prop.comment ? `${transformComment(prop.comment)}` : '',
            propDeclaration
        ], false);
    }
