...
beforeAll(async () => {
    ...
    s3client = new AWS.S3({
        ...
        endpoint: ep.href,
        s3BucketEndpoint: true, sslEnabled: false, s3ForcePathStyle: true,
    });
...
describe('Test list api', () => {
    ...
    test('list object pagination', async () => {
        let maxKeys = 2;
        let total = 7;
        let response = await s3client.listObjects({
            Bucket: bucket, Prefix: 'pagination',
            Marker: '', MaxKeys: maxKeys
        }).promise();
        while (response.IsTruncated === true) {
            ...
            response = await s3client.listObjects({
                Bucket: bucket, Prefix: 'pagination',
                Marker: response.NextMarker, MaxKeys: maxKeys
            }).promise();
            total = total - maxKeys;
        }
