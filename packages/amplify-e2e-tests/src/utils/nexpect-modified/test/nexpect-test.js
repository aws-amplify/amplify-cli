...
vows
  ...
  .addBatch({
    'When using the nexpect module': {
      ...
      spawning: {
        '`echo hello`': assertSpawn(nexpect.spawn('echo', ['hello']).expect('hello')),
        '`ls -l /tmp/undefined`': assertSpawn(
          nexpect.spawn('ls -la /tmp/undefined', { stream: 'stderr' }).expect('No such file or directory')
        ),
        'a command that does not exist': assertError(nexpect.spawn('idontexist').expect('This will never work')),
        ...
        'and using the expect() method': {
          'when RegExp expectation is met': assertSpawn(nexpect.spawn('echo', ['hello']).expect(/^hello$/)),
        },
          ...
          'when a callback is provided and output is not matched': {
            topic: function() {
              ...
              var expect = nexpect
                ...
                .wait('second')
                .sendline('second-prompt')
                .wait('this-never-shows-up', waitCallback)
                .run(this.callback.bind(this, args));
            },
        ...
        'when options.stripColors is set': assertSpawn(
          nexpect
            .spawn(path.join(__dirname, 'fixtures', 'log-colors'), { stripColors: true })
            .wait('second has colors')
            .expect('third has colors')
        ),
        'when options.ignoreCase is set': assertSpawn(
          nexpect
            .spawn(path.join(__dirname, 'fixtures', 'multiple-cases'), { ignoreCase: true })
            .wait('this has many cases')
            .expect('this also has many cases')
        ),
        'when options.cwd is set': assertSpawn(
          nexpect
            .spawn(path.join(__dirname, 'fixtures', 'show-cwd'), { cwd: path.join(__dirname, 'fixtures') })
            .wait(path.join(__dirname, 'fixtures'))
        ),
        'when options.env is set': assertSpawn(
          nexpect.spawn(path.join(__dirname, 'fixtures', 'show-env'), { env: { foo: 'bar', PATH: process.env.PATH } }).expect('foo=bar')
        ),
      },
    },
  })
