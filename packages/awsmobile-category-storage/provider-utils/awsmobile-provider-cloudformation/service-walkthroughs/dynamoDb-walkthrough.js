...
async function serviceWalkthrough(context, defaultValuesFilename, serviceMetadata) {
  ...
  const resourceQuestions = [
    {
      ...
      validate: awsmobile.inputValidation(inputs[0]),
      default: () => {
        const defaultValue = getAllDefaults(awsmobile.getProjectDetails())[inputs[0].key];
        return defaultValue;
      },
    },
    {
      ...
      validate: awsmobile.inputValidation(inputs[1]),
  ...
  const attributeQuestion = {
    ...
    validate: awsmobile.inputValidation(inputs[2]),
  };
  ...
  const primaryKeyQuestion = {
    ...
    validate: awsmobile.inputValidation(inputs[3]),
