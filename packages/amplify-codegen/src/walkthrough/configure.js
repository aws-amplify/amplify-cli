...
async function configureProjectWalkThrough(context, amplifyConfig) {
  ...
  const selectedProjectConfig = deepCopy(amplifyConfig.find(project => (
    project.amplifyExtension.graphQLApiId === apiId
  )))

  const { amplifyExtension } = selectedProjectConfig
  selectedProjectConfig.includes = await askCodeGeneQueryFilePattern(selectedProjectConfig.includes)
