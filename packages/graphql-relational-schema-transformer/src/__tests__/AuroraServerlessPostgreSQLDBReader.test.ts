...
const dummyReader = new AuroraServerlessPostgreSQLDatabaseReader(dbRegion, secretStoreArn, clusterArn, testDBName, aws);
...
test('Test hydrate template context', async () => {
  ...
  expect(context.databaseSchema).toEqual('postgresql');
});
