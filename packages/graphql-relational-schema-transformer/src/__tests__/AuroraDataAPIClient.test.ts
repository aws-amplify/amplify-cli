...
test('Test list tables', async () => {
   ...
   const MockRDSClient = jest.fn<any>(() => ({
      executeSql: jest.fn((params: DataApiParams) => {
         if (params.sqlStatements == 'SHOW TABLES') {
            return rdsPromise
         }
   ...
   Params.awsSecretStoreArn = secretStoreArn
   Params.dbClusterOrInstanceArn = clusterArn
   Params.database = databaseName
   Params.sqlStatements = 'SHOW TABLES'
   expect(mockRDS.executeSql).toHaveBeenCalledWith(Params)
...
test('Test foreign key lookup', async() => {
   ...
   const MockRDSClient = jest.fn<any>(() => ({
      executeSql: jest.fn((params: DataApiParams) => {
         if (params.sqlStatements.indexOf(`AND REFERENCED_TABLE_NAME = '${tableBName}'`) > -1) {
            return rdsPromise
         }
         throw new Error('Incorrect SQL given.')
      })
...
test('Test describe table', async() => {
   ...
   const MockRDSClient = jest.fn<any>(() => ({
      executeSql: jest.fn((params: DataApiParams) => {
         if (params.sqlStatements == `DESCRIBE ${tableAName}`) {
            return rdsPromise
         }
         throw new Error('Incorrect SQL given.')
      })
   ...
   Params.dbClusterOrInstanceArn = clusterArn
   Params.database = databaseName
   Params.sqlStatements = `DESCRIBE ${tableAName}`
   expect(mockRDS.executeSql).toHaveBeenCalledWith(Params)
