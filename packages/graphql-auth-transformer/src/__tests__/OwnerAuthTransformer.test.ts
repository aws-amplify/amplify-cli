...
test('Test OwnerField with a list', () => {
  const validSchema = `
  type Chat @model
      @auth(rules: [
  ...
  // expect 'owners' as an argument for subscription operations
  expect(out.schema).toContain('onCreateChat(owners: [String]!)');
  expect(out.schema).toContain('onUpdateChat(owners: [String]!)');
  expect(out.schema).toContain('onDeleteChat(owners: [String]!)');

  // expect logic in the resolvers to check for postOwner args as an allowerOwner
  expect(out.resolvers['Subscription.onCreateChat.res.vtl']).toContain(
    '#set( $allowedOwners0 = $util.defaultIfNull($ctx.args.owners, null) )'
  );
  expect(out.resolvers['Subscription.onUpdateChat.res.vtl']).toContain(
    '#set( $allowedOwners0 = $util.defaultIfNull($ctx.args.owners, null) )'
  );
  expect(out.resolvers['Subscription.onDeleteChat.res.vtl']).toContain(
    '#set( $allowedOwners0 = $util.defaultIfNull($ctx.args.owners, null) )'
  );
});
