...
import {
  ...
  makeField,
  ModelResourceIDs,
  KeyDirectiveArguments,
  shouldKeyDirectiveGenerateQuery,
  getKeyDirectiveQueryFieldName,
...
operations will be generated by the CLI.`,
  ...
  private protectQueries(
    ...
    const secondaryKeyDirectivesWithQueries = (def.directives || []).filter(d => {
      ...
      const args: KeyDirectiveArguments = getDirectiveArguments(d);
      // @key with a name is a secondary key.
      const isSecondaryKey = Boolean(args.name);
      const shouldGenerateQuery = shouldKeyDirectiveGenerateQuery(args);
      return isKey && isSecondaryKey && shouldGenerateQuery;
    });
    for (const keyWithQuery of secondaryKeyDirectivesWithQueries) {
      const args: KeyDirectiveArguments = getDirectiveArguments(keyWithQuery);
      const queryField = getKeyDirectiveQueryFieldName(def, args);
      const resolverResourceId = ResolverResourceIDs.ResolverResourceID(ctx.getQueryTypeName(), queryField);
      this.protectListQuery(ctx, resolverResourceId, rules, null, modelConfiguration, queryField);
    }
  }
