...
const serviceName = AmplifySupportedService.PINPOINT;
 ...
 * @returns resource
 */
const addWalkthrough = async (context : $TSContext, defaultValuesFilename: string, serviceMetadata: $TSAny): Promise<$TSAny> => {
 ...
 * @param {*} resourceDirPath Path to params.json
 */
export const readParams = (resourceDirPath: string): Record<string, $TSAny> => {
  fs.ensureDirSync(resourceDirPath);
  const parametersFilePath = path.join(resourceDirPath, parametersFileName);
 ...
 */
const getIAMPolicies = (resourceName: string, crudOptions: $TSAny): $TSAny => {
  let policy = {};
