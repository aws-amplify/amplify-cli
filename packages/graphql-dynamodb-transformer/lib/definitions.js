...
function makeCreateInputObject(obj) {
    var name = graphql_transformer_common_1.graphqlName("Create" + graphql_transformer_common_1.toUpper(obj.name.value) + 'Input');
...
function makeUpdateInputObject(obj) {
    var name = graphql_transformer_common_1.graphqlName('Update' + graphql_transformer_common_1.toUpper(obj.name.value) + 'Input');
    var fields = obj.fields
        .map(function (field) { return ({
        kind: graphql_1.Kind.INPUT_VALUE_DEFINITION,
        name: field.name,
        type: field.name.value === 'id' ?
            graphql_transformer_common_1.wrapNonNull(field.type) :
            graphql_transformer_common_1.unwrapNonNull(field.type),
        // TODO: Service does not support new style descriptions so wait.
        // description: field.description,
        directives: []
    }); });
...
function makeDeleteInputObject(obj) {
    var name = graphql_transformer_common_1.graphqlName('Delete' + graphql_transformer_common_1.toUpper(obj.name.value) + 'Input');
    return {
        kind: graphql_1.Kind.INPUT_OBJECT_TYPE_DEFINITION,
        // TODO: Service does not support new style descriptions so wait.
        // description: {
...
function makeTableXFilterInputObject(obj) {
    var name = graphql_transformer_common_1.graphqlName("Table" + obj.name.value + "FilterInput");
    var fields = obj.fields
        .filter(function (field) { return graphql_transformer_common_1.isScalar(field.type) === true; })
        .map(function (field) { return ({
        kind: graphql_1.Kind.INPUT_VALUE_DEFINITION,
        name: field.name,
        type: graphql_transformer_common_1.makeNamedType('Table' + graphql_transformer_common_1.getBaseType(field.type) + 'FilterInput'),
        // TODO: Service does not support new style descriptions so wait.
        // description: field.description,
        directives: []
    }); });
...
function makeTableSortDirectionEnumObject() {
    var name = graphql_transformer_common_1.graphqlName('TableSortDirection');
    return {
...
function makeTableScalarFilterInputObject(type) {
    var name = graphql_transformer_common_1.graphqlName("Table" + type + "FilterInput");
