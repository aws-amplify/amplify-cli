...
import { getAmplifyUIBuilderService } from './utils/uibuilder-client';
...
export async function run(context: $TSContext) {
  ...
  const amplifyUIBuilder = await getAmplifyUIBuilderService(context, newEnvName, appId);
  ...
  const promises = components.map(async (component: any) => {
    return await amplifyUIBuilder.createComponent({
      appId,
      environmentName,
      componentToCreate: {
