...
import { $TSMeta, $TSTeamProviderInfo, DeploymentSecrets, HooksConfig } from '..';
import { JSONUtilities } from '@aws-amplify/amplify-json-utilties';
...
export class StateManager {
  ...
  setDeploymentSecrets = (deploymentSecrets: DeploymentSecrets): void => {
    ...
    JSONUtilities.writeJson(deploymentSecretsPath, deploymentSecrets, { mode: SECRET_FILE_MODE }); // set deployment secret file permissions to -rw-------
  };
  ...
  getLocalEnvInfo = (projectPath?: string, options?: GetOptions<any>): any => {
  ...
  getProjectConfig = (projectPath?: string, options?: GetOptions<any>): any => {
   ...
   */
  getBackendConfig = (projectPath?: string, options?: GetOptions<any>, includeParameters = false): any => {
    ...
    const data = this.getData<any>(filePath, mergedOptions);
  ...
  getCurrentBackendConfig = (projectPath?: string, options?: GetOptions<any>): any => {
  ...
  getResourceParametersJson = (projectPath: string | undefined, category: string, resourceName: string, options?: GetOptions<any>): any => {
  ...
  getResourceInputsJson = (projectPath: string | undefined, category: string, resourceName: string, options?: GetOptions<any>): any => {
  ...
  getAmplifyAdminConfigEntry = (appId: string, options?: GetOptions<any>): any => {
    const mergedOptions = {
      throwIfNotExist: false,
      default: {},
      ...options,
    ...
    const adminConfig =
      JSONUtilities.readJson<any>(pathManager.getAmplifyAdminConfigFilePath(), { throwIfNotExist: false }) ?? mergedOptions.default;

    return adminConfig[appId];
  };

  removeAmplifyAdminConfigEntry = (appId: string): void => {
    const adminConfig: any = JSONUtilities.readJson(pathManager.getAmplifyAdminConfigFilePath());
    delete adminConfig[appId];
    JSONUtilities.writeJson(pathManager.getAmplifyAdminConfigFilePath(), adminConfig, { secureFile: true });
  };

  setAmplifyAdminConfigEntry = (appId: string, config: any): void => {
    const adminConfig: any = JSONUtilities.readJson(pathManager.getAmplifyAdminConfigFilePath(), { throwIfNotExist: false }) || {};
    adminConfig[appId] = config;
    JSONUtilities.writeJson(pathManager.getAmplifyAdminConfigFilePath(), adminConfig, { secureFile: true });
  };

  setLocalEnvInfo = (projectPath: string | undefined, localEnvInfo: any): void => {
    const filePath = pathManager.getLocalEnvFilePath(projectPath);

    JSONUtilities.writeJson(filePath, localEnvInfo);
  };

  setLocalAWSInfo = (projectPath: string | undefined, localAWSInfo: any): void => {
    const filePath = pathManager.getLocalAWSInfoFilePath(projectPath);
  ...
  setProjectConfig = (projectPath: string | undefined, projectConfig: any): void => {
    const filePath = pathManager.getProjectConfigFilePath(projectPath);
  ...
  setBackendConfig = (projectPath: string | undefined, backendConfig: any): void => {
    const filePath = pathManager.getBackendConfigFilePath(projectPath);

    JSONUtilities.writeJson(filePath, backendConfig, { orderedKeys: true });
  };

  setCurrentBackendConfig = (projectPath: string | undefined, backendConfig: any): void => {
    const filePath = pathManager.getCurrentBackendConfigFilePath(projectPath);
  ...
  setResourceParametersJson = (projectPath: string | undefined, category: string, resourceName: string, parameters: any): void => {
    const filePath = pathManager.getResourceParametersFilePath(projectPath, category, resourceName);

    JSONUtilities.writeJson(filePath, parameters);
  };

  setResourceInputsJson = (
    projectPath: string | undefined,
    category: string,
    resourceName: string,
  ...
  getCLIJSON = (projectPath: string, env?: string, options?: GetOptions<any>): any => {
