...
exports.generateHostingResources = void 0;
...
async function generateHostingResources(context, { domain, restrictAccess, hostedZoneId, exposedContainer: exposedContainerFromMeta = undefined, dependsOn: dependsOnFromRolePermissions = [], categoryPolicies = [], environmentMap = {}, mutableParametersState = {}, }, addResource = false) {
    const dependsOn = [];
    let authName;
    let authDependensOn;
    if (restrictAccess) {
    ...
    const { auth: authDependsOnFromRolePermissions, rest: restDependsOnRolePermissions } = dependsOnFromRolePermissions.reduce((acc, dep) => {
    ...
    const { providers: { [constants.providerName]: provider }, } = context.amplify.getProjectMeta();
    ...
    const { containers, containersPorts, desiredCount, exposedContainer, isInitialDeploy, secretsArns } = await processDockerConfig(context, resource, srcPath);
...
exports.generateHostingResources = generateHostingResources;
...
async function configure(context) {
    const { hosting: { ElasticContainer: { domain: currentDomain, restrictAccess: currentRestrictAccess, mutableParametersState: currentMutableParameterState, environmentMap: currentEnvironmentMap, }, }, } = context.amplify.getProjectMeta();
