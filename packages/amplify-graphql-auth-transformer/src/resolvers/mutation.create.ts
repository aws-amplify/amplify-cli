      ...
      ownerExpression.push(
        iff(
          ...
          compoundExpression([
            set(
              ref(`ownerEntity${idx}`),
              methodCall(ref('util.defaultIfNull'), ref(`ctx.input.args.${role.entity!}`), entityIsList ? list([]) : nul()),
            ),
            ...
            ...(entityIsList
              ...
              : [iff(equals(ref(`ownerClaim${idx}`), ref(`ownerClaim${idx}`)), addAllowedFieldsIfElse(`ownerAllowedFields${idx}`))]),
            iff(
              and([isNullOrEmpty(ref(`ownerEntity${idx}`)), not(methodCall(ref('ctx.args.input.containsKey'), ref(`${role.entity!}`)))]),
              compoundExpression([
                qref(
                  methodCall(
