...
export const invokeAnalyticsAPICreateResource = async (context: $TSContext, resourceProviderServiceName: string):
  Promise<IAmplifyResource> => (await context.amplify.invokePluginMethod(context,
    'analytics', undefined,
 ...
 */
export const invokeGetLastPushTimeStamp = async (
  amplifyMeta: $TSMeta,
  analyticsResourceName: string,
): Promise<string|undefined> => amplifyMeta[AmplifyCategories.ANALYTICS][analyticsResourceName].lastPushTimeStamp;
 ...
 */
export const invokeAnalyticsPush = async (
  context: $TSContext,
  analyticsResourceName: string,
): Promise<IPluginCapabilityAPIResponse> => (await context.amplify.invokePluginMethod(context,
