...
import { generateAuthStackTemplate } from '../..';
...
export const run = async (context: $TSContext) => {
  ...
  if (authResources.length >= 1) {
    ...
    try {
      const cliState = new AuthInputState(resourceAnswer.resource);
      cliInputs = cliState.getCLIInputPayload();
    } catch (err) {
      printer.warn('Cli-inputs.json doesnt exist');
      const isMigrate = await prompter.confirmContinue(`Do you want to migrate this ${resourceAnswer.resource} to support overrides?`);
      if (isMigrate) {
        migrateResourceToSupportOverride(context, resourceAnswer.resource);
        generateAuthStackTemplate(context, resourceAnswer.resource);
      } else {
    ...
    const destPath = path.normalize(path.join(backendDir, category, resourceAnswer.resource));
