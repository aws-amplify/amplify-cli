...
import { FeatureFlags, FeatureFlagRegistration, FeatureFlagsEntry, FeatureFlagType } from '.';
...
export class FeatureFlagProvider {
  ...
  public static initialize = async (environmentProvider: CLIEnvironmentProvider, projectPath: string): Promise<FeatureFlagProvider> => {
    ...
    const instance = new FeatureFlagProvider(environmentProvider, projectPath);
    ...
    instance.effectiveFlags = _.merge(
      {},
      fileFlags.project,
      fileFlags.environments[environmentProvider.getCurrent()] ?? {},
      envFlags.project,
      envFlags.environments[environmentProvider.getCurrent()] ?? {},
    );

    FeatureFlagProvider.instance = instance;
  ...
  public static ensureDefaultFeatureFlags = async (newProject: boolean): Promise<void> => {
    FeatureFlagProvider.ensureInitialized();

    const configFileName = path.join(FeatureFlagProvider.instance.projectPath, AmplifyConfigFileName);
    ...
    if (!config || (config && !config.features)) {
      config = {
        ...config,
        features: newProject ? FeatureFlagProvider.getNewProjectDefaults() : FeatureFlagProvider.getExistingProjectDefaults(),
      };
  ...
  private static ensureInitialized = (): void => {
    if (!FeatureFlagProvider.instance) {
      throw new Error('FeatureFlagsProvider is not initialized');
    }
  };

  public static getBoolean = (flagName: string): boolean => {
    return FeatureFlagProvider.getValue<boolean>(flagName, 'boolean');
  };

  public static getString = (flagName: string): string => {
    return FeatureFlagProvider.getValue<string>(flagName, 'string');
  };

  public static getNumber = (flagName: string): number => {
    return FeatureFlagProvider.getValue<number>(flagName, 'number');
  };

  public static getEffectiveFlags = (): Readonly<FeatureFlagsEntry> => {
    FeatureFlagProvider.ensureInitialized();

    return FeatureFlagProvider.instance.effectiveFlags;
  };

  public static getNewProjectDefaults = (): Readonly<FeatureFlagsEntry> => {
    FeatureFlagProvider.ensureInitialized();

    return FeatureFlagProvider.instance.newProjectDefaults;
  };

  public static getExistingProjectDefaults = (): Readonly<FeatureFlagsEntry> => {
    FeatureFlagProvider.ensureInitialized();

    return FeatureFlagProvider.instance.existingProjectDefaults;
  };

  private static getValue = <T>(flagName: string, type: FeatureFlagType): T => {
    FeatureFlagProvider.ensureInitialized();
    ...
    // Get registration
    const sectionRegistration = FeatureFlagProvider.instance.registrations.get(parts[0]);
    ...
    if (FeatureFlagProvider.instance.effectiveFlags[parts[0]] && FeatureFlagProvider.instance.effectiveFlags[parts[0]][parts[1]]) {
      value = <T>FeatureFlagProvider.instance.effectiveFlags[parts[0]][parts[1]];
    }
  ...
  private validateFlags = (allFlags: { name: string; flags: FeatureFlags }[]): void => {
    ...
    const featureFlagsValidator = (type: string, features: FeatureFlags) => {
  ...
  private transformEnvFlags = (features: FeatureFlags): FeatureFlags => {
