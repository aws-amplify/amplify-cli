...
const TMP_ROOT = '/tmp/model_transform_tests_v2/';
...
beforeAll(async () => {
  ...
  const finishedStack = await deploy(customS3Client, cf, STACK_NAME, out, {}, TMP_ROOT, BUCKET_NAME, ROOT_KEY, BUILD_TIMESTAMP);
...
test('Test updateComment mutation with null and empty', async () => {
  ...
  expect(update2Response.errors[0].message).toEqual(
    "Cannot return null for non-nullable type: 'String' within parent 'Require' (/updateRequire/requiredField)",
  );
});
...
test('Test enum filters List', async () => {
  ...
  console.warn(appearsInWithFilterResponseJedi);
