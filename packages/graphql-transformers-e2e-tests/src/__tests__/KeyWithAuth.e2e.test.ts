...
test('Test only authorized user can see results in custom queries generated by @key directive', async () => {
  ...
  const querResponse1 = await queryPostByAuthor(GRAPHQL_CLIENT_1, 'author1');
  expect(querResponse1.data.queryPostByAuthor).toBeDefined();
  expect(querResponse1.data.queryPostByAuthor.items).toHaveLength(1);
  expect(querResponse1.data.queryPostByAuthor.items[0].title).toEqual('by author1');
  expect(querResponse1.data.queryPostByAuthor.items[0].author).toEqual('author1');

  const querResponse2 = await queryPostByAuthor(GRAPHQL_CLIENT_1, 'author2');
  expect(querResponse2.data.queryPostByAuthor).toBeDefined();
  expect(querResponse2.data.queryPostByAuthor.items).toHaveLength(0);

  const querResponse3 = await queryPostByAuthor(GRAPHQL_CLIENT_2, 'author2');
  expect(querResponse3.data.queryPostByAuthor).toBeDefined();
  expect(querResponse3.data.queryPostByAuthor.items).toHaveLength(1);
  expect(querResponse3.data.queryPostByAuthor.items[0].title).toEqual('by author2');
  expect(querResponse3.data.queryPostByAuthor.items[0].author).toEqual('author2');

  const querResponse4 = await queryPostByAuthor(GRAPHQL_CLIENT_2, 'author1');
  expect(querResponse4.data.queryPostByAuthor).toBeDefined();
  expect(querResponse4.data.queryPostByAuthor.items).toHaveLength(0);
});
...
async function queryPostByAuthor(client: GraphQLClient, author: string) {
  const result = await client.query(
    /* GraphQL */ `
      query queryPostByAuthor($author: String!) {
        queryPostByAuthor(author: $author) {
          items {
            author
            createdAt
            id
