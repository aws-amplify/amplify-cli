...
import {
    createUserPool, createUserPoolClient, deleteUserPool, addIAMRolesToCFNStack,
...
test('test Comments as owner', async () => {
    const ownerResponse: any = await GRAPHQL_CLIENT_1.query({query: gql`
...
test('test Comments as user in writer group', async () => {
    const writerResponse: any = await GRAPHQL_CLIENT_2.query({ query: gql`
        query SearchComments {
            searchComments {
            items {
                id
...
test('test Comments as user that is not an owner nor is in writer group', async () => {
    const user3Response: any = await GRAPHQL_CLIENT_3.query({query: gql`
        query SearchComments {
            searchComments {
            items {
                id
...
test('test Todo as user in the dynamic group admin', async () => {
    const adminResponse: any = await GRAPHQL_CLIENT_2.query({ query: gql`
        query SearchTodos {
            searchTodos {
            items {
                id
...
test('test Post as authorized user', async () => {
    const authUser: any = await GRAPHQL_IAM_AUTH_CLIENT.query({ query: gql`
        query SearchPosts {
            searchPosts{
                items {
                    id
...
test('test searchPosts with apikey and secret removed', async () => {
    const apiKeyResponse: any = await GRAPHQL_APIKEY_CLIENT.query({ query: gql`
            query SearchPosts {
                searchPosts{
                    items {
                        id
...
test('test post as an cognito user that is not allowed in this schema', async () => {
    try {
        await GRAPHQL_CLIENT_3.query({query: gql`
        query SearchPosts {
            searchPosts{
                items {
                    id
...
async function createComment(client: AWSAppSyncClient<any>, input: CreateCommentInput) {
    const create = gql`mutation CreateComment($input: CreateCommentInput!) {
...
async function createTodo(client: AWSAppSyncClient<any>, input: CreateTodoInput) {
    const create = gql`mutation CreateTodo($input: CreateTodoInput!) {
...
async function createPost(client: AWSAppSyncClient<any>, input: CreatePostInput) {
    const create = gql`mutation CreatePost($input: CreatePostInput!) {
...
async function createRecords() {
    ...
    try {
        ...
        const createCommentList: CreateCommentInput[] = [
            { content: 'content1' },
            { content: 'content2' },
            { content: 'content3' }
        ]
