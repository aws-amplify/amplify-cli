...
import {
  ...
  geoServiceExists,
  getGeoPricingPlan,
  checkGeoResourceExists,
} from '../service-utils/resourceUtils';
import {
  resourceAccessWalkthrough,
  pricingPlanWalkthrough,
  dataProviderWalkthrough,
  getServiceFriendlyName,
  defaultResourceQuestion,
} from './resourceWalkthrough';
...
export const createPlaceIndexWalkthrough = async (
  ...
  if (currentPlaceIndexResources && Object.keys(currentPlaceIndexResources).length > 0) {
    parameters.isDefault = await prompter.yesOrNo(defaultResourceQuestion(ServiceName.PlaceIndex), true);
  } else {
...
export const placeIndexNameWalkthrough = async (context: any): Promise<Partial<PlaceIndexParameters>> => {
  ...
  while (!indexName) {
    ...
    const indexNameInput = await prompter.input('Provide a name for the location search index (place index):', {
...
export const placeIndexDataStorageWalkthrough = async (
  context: $TSContext,
  parameters: Partial<PlaceIndexParameters>,
): Promise<Partial<PlaceIndexParameters>> => {
...
export const updatePlaceIndexWalkthrough = async (
  ...
  const indexResources = ((await context.amplify.getResourceStatus()).allResources as any[]).filter(
    resource => resource.service === ServiceName.PlaceIndex,
  );

  if (indexResources.length === 0) {
    printer.error(
      `No ${searchServiceFriendlyName} resource to update. Use "amplify add geo" to create a new ${searchServiceFriendlyName}.`,
    );
    return parameters;
  }
...
export const updateDefaultPlaceIndexWalkthrough = async (
  ...
  if (otherIndexResources?.length > 0) {
    const defaultIndexName = await prompter.pick(
      `Select the ${searchServiceFriendlyName} you want to set as default:`,
      otherIndexResources,
    );
