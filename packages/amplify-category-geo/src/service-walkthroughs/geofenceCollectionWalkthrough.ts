...
export const createGeofenceCollectionWalkthrough = async (
  ...
  if (currentGeofenceCollectionResources && Object.keys(currentGeofenceCollectionResources).length > 0) {
    parameters.isDefault = await prompter.yesOrNo(defaultResourceQuestion(ServiceName.GeofenceCollection), true);
  } else {
...
export const geofenceCollectionNameWalkthrough = async (): Promise<Pick<GeofenceCollectionParameters, 'name'>> => {
  ...
  const uniquenessValidation: Validator = async (input: string) =>
    (await checkGeoResourceExists(input)) ? `Geo resource ${input} already exists. Choose another name.` : true;
  const validator = and([
  ...
  const collectionNameInput = await prompter.input('Provide a name for the Geofence Collection:', {
    validate: validator,
...
export const geofenceCollectionAccessWalkthrough = async (
  ...
  if (userPoolGroupList.length <= 0) {
    if (
      await prompter.yesOrNo('Geofencing requires a Cognito user group for Admin only access control settings. Do you want to add it now?')
    ) {
      ...
      printer.error(
        'No Cognito groups exist in the project. Please add a Cognito group using "amplify update auth" and selecting "Create or update Cognito user pool groups"',
      );
...
export const updateGeofenceCollectionWalkthrough = async (
  ...
  if (collectionResourceNames.length === 0) {
    printer.error(
      `No ${geofencingServiceFriendlyName} resource to update. Use "amplify add geo" to create a new ${geofencingServiceFriendlyName}.`,
    );
    return parameters;
  }
  ...
  if (resourceToUpdate) {
    ...
    resourceToUpdate = await prompter.pick<'one', string>(
      `Select the ${geofencingServiceFriendlyName} you want to update`,
...
export const updateDefaultGeofenceCollectionWalkthrough = async (
  ...
  const otherCollectionResources = availableGeofenceCollections.filter(
    (collectionResourceName) => collectionResourceName !== currentDefault,
  );
  if (otherCollectionResources?.length > 0) {
    const defaultIndexName = await prompter.pick(
      `Select the ${geofencingServiceFriendlyName} you want to set as default:`,
      otherCollectionResources,
    );
