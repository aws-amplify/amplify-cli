...
import {
  ...
  GeofenceParams,
  IdentifierOption
} from "../service-utils/populateParams";
...
export const importResource = async (context: $TSContext) => {
  const geofenceCollectionResources = ((await context.amplify.getResourceStatus()).allResources as any[]).filter(
    resource => resource.service === ServiceName.GeofenceCollection
  );
  ...
  const geoJSONFilePath = join(
    await prompter.input(
      `Provide the path to GeoJSON file containing the Geofences for ${collectionToPopulate} collection. Refer https://geojson.io/ for a sample GeoJSON:`
    )
  );
  ...
  const identifierWalkthroughOptions = [
    {
      name: 'No I will use the root level "id" field on Feature type. Auto-Assign if missing (this will UPDATE the GeoJSON file)',
      value: IdentifierOption.RootLevelID
    },
    { name: "Yes I want to use one of the Geofence(Feature) properties as an identifier", value: IdentifierOption.CustomProperty }
  ];
  const identifierOption = (await prompter.pick<"one", string>(
  ...
  if (identifierOption === IdentifierOption.CustomProperty) {
    uniqueIdentifier = await prompter.input(
...
const constructGeofenceCollectionParams = (populateParam: PopulateParams): GeofenceCollectionParams => {
  ...
  geoJSONObj.features.forEach(feature => {
    Entries.push({
      GeofenceId:
        populateParam.identifierOption === IdentifierOption.CustomProperty
