...
describe('cdk stack creation for geofence collection service', () => {
  ...
  it('creates geofence collection policy for given group permissions', async () => {
    const stackProps = {
      ...
      groupPermissions: {
        collectionsAdmin: ['Read geofence', 'Create/Update geofence'],
      },
      authResourceName: 'mockAuthResource123',
    };
    const geofenceCollectionStack = new GeofenceCollectionStack(new App(), 'GeofenceCollectionStack', stackProps);
    const geofenceCollectionStackGroupAccessPolicy =
      geofenceCollectionStack.toCloudFormation().Resources.collectionsAdminGeofenceCollectionPolicy;
    expect(geofenceCollectionStackGroupAccessPolicy).toBeDefined();
    const geofenceCollectionStackGroupAccessPolicyActions =
      geofenceCollectionStackGroupAccessPolicy.Properties.PolicyDocument.Statement[0].Action;
