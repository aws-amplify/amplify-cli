...
describe("Geo JSON file validation test", () => {
  ...
  it("should throw error for a invalid Geo JSON file with clockwise exterior ring", () => {
    expect(() =>
      validateGeoJSONFile(path.join(assetRoot, "invalidGeofence.clockwiseExteriorRing.json"))
    ).toThrowErrorMatchingInlineSnapshot(`"The first linear ring is exterior ring and should be counter-clockwise."`);
  });
  it("should throw error if custom identifier is not found", () => {
    expect(() =>
      validateGeoJSONFile(path.join(assetRoot, "validGeofence.nameAsIdentifier.json"), "id", IdentifierOption.CustomProperty)
  ...
  it("should throw error for a invalid Geo JSON file with counter-clockwise interior ring", () => {
    expect(() =>
      validateGeoJSONFile(path.join(assetRoot, "invalidGeofence.counterclockwiseInteriorRing.json"))
