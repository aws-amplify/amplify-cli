...
describe('Map walkthrough works as expected', () => {
  ...
  beforeEach(() => {
    ...
    mockContext.amplify.getResourceStatus = jest
      .fn()
      .mockImplementation((category?: any, resourceName?: any, providerName?: any, filteredResources?: any): Promise<any> => {
  ...
  it('updates default map to another map if it is removed', async () => {
    mockContext.amplify.removeResource = jest.fn().mockReturnValue({
      then: jest.fn().mockReturnValue(await updateDefaultMapWalkthrough(mockContext, mockMapName)),
    });
