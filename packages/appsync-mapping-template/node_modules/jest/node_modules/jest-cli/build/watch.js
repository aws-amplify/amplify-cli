...
function _load_get_changed_files_promise() {
  return (_get_changed_files_promise = _interopRequireDefault(
    require('./get_changed_files_promise')
  ));
}
...
function _load_failed_tests_cache() {
  return (_failed_tests_cache = _interopRequireDefault(
    require('./failed_tests_cache')
  ));
}
...
function watch(initialGlobalConfig, contexts, outputStream, hasteMapInstances) {
  ...
  let hooks =
    arguments.length > 5 && arguments[5] !== undefined
      ? arguments[5]
      : new (_jest_hooks || _load_jest_hooks()).default();
  ...
  hasteMapInstances.forEach((hasteMapInstance, index) => {
    hasteMapInstance.on('change', _ref3 => {
      ...
      if (validPaths.length) {
        ...
        searchSources[index] = {
          context,
          searchSource: new (_search_source || _load_search_source()).default(
            context
          )
        };
        emitFileChange();
  ...
  const startRun = globalConfig => {
    ...
    testWatcher = new (_test_watcher || _load_test_watcher()).default({
      isWatchMode: true
    });
    ...
    (_get_changed_files_promise || _load_get_changed_files_promise()).default)(
      globalConfig,
      configs
    );
  ...
  const onKeypress = key => {
    if (
      key === (_constants || _load_constants()).KEYS.CONTROL_C ||
      key === (_constants || _load_constants()).KEYS.CONTROL_D
    ) {
    ...
    if (
      ...
      testWatcher &&
      [
        (_constants || _load_constants()).KEYS.Q,
        (_constants || _load_constants()).KEYS.ENTER,
        (_constants || _load_constants()).KEYS.A,
        (_constants || _load_constants()).KEYS.O,
        (_constants || _load_constants()).KEYS.F
      ]
        .concat(pluginKeys)
        .indexOf(key) !== -1
    ) {
    ...
    (_watch_plugins_helpers || _load_watch_plugins_helpers())
      .filterInteractivePlugins)(watchPlugins, globalConfig).find(plugin => {
      const usageData =
