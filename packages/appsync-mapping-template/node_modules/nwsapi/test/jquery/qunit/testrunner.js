 ...
 *
 * Modified to run selector test standalone (diego.perini@gmail.com)
 *
 */
...
if (location.search.length > 1) {
	...
	var parms = location.search.slice(1).split('&');
	for (var i = 0; parms.length > i; i++) {
		parms[i] = decodeURIComponent(parms[i]);
	}
	_config.filters = parms;
} else {
	_config.filters = false;
}
...
//	_config.ajaxSettings = jQuery.ajaxSettings;
	synchronize(function() {
		var output =
			'<p id="testresult" class="result">' +
			['Tests completed in ', +new Date - started, ' milliseconds.<br/>',
			'<span class="bad">', _config.stats.bad, '</span> tests of <span class="all">',
			_config.stats.all, '</span> failed.</p>'].join('');
		document.body.appendChild(toDOMFragment(output));
	...
	synchronize(function() {
        ...
        if (b.addEventListener)
			b.addEventListener('dblclick', function(event) {
				var target = event.target.parentNode.getElementsByTagName("strong");
				if ( target.length ) {
					location.href = location.href.match(/^(.+?)(\?.*)?$/)[1] + "?" + encodeURIComponent(target[0].textContent);
				}
			}, false);
		else
			b.attachEvent('ondblclick', function(event) {
				var target = event.srcElement.parentNode.getElementsByTagName("strong");
				if ( target.length ) {
					location.href = location.href.match(/^(.+?)(\?.*)?$/)[1] + "?" + encodeURIComponent(target[0].innerText);
				}
			}, false);
...
/**
 * Asserts true.
 * @example ok( $("a").length > 5, "There must be at least 5 anchors" );
 */
function ok(a, msg) {
