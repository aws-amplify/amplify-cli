  ...
  if (startChoice[inputs[1].key] === 'Start with a sample') {
    ...
    if (intentChoice[inputs[6].key] === 'Update an existing intent') {
      ...
      if (addUtteranceAnswer[inputs[8].key]) {
        utterances = await addUtterance(context, serviceMetadata);
      }
      ...
      if (addSlotAnswer[inputs[9].key]) {
        slotReturn = await addSlot(context, intentName, serviceMetadata, parameters);
      }
      ...
      while (continueAddingIntents) {
        intents.push(await addIntent(context, serviceMetadata, intents, parameters));
  ...
  if (parameters) {
    if (answers.intentName) {
      if (deleteIntentConfirmed) {
        parameters.intents = parameters.intents.filter(i => i.intentName !== answers.intentName);
      } else {
...
async function addIntent(context, serviceMetadata, intents, parameters) {
  ...
  while ((intents.filter(intent => intent.intentName === intentName).length > 0) ||
        (parameters && parameters.intents.filter(i => i.intentName === intentName).length > 0)) {
    print.info('');
  ...
  const utterances = await addUtterance(context, serviceMetadata);
  ...
  let slots = [];
  let newSlotTypes = [];
  const slotReturn = await addSlot(context, intentName, serviceMetadata, parameters);
  if (slotReturn.length > 1) {
    newSlotTypes = slotReturn[1];
  }
  slots = slotReturn[0];
...
async function addUtterance(context, serviceMetadata) {
  const { inputs } = serviceMetadata;
  const { amplify } = context;
...
async function addSlot(context, intentName, serviceMetadata, parameters) {
  const { inputs } = serviceMetadata;
  const { amplify, print } = context;
  ...
  while (addAnotherSlot) {
    ...
    while ((slots.filter(existingSlot => existingSlot.name === slot.name).length > 0) ||
          (parameters && parameters.intents.filter(i => i.intentName === intentName)[0] &&
          parameters.intents.filter(i => i.intentName === intentName)[0]
            .slots.filter(s => s.name === slot.name).length > 0)) {
      print.info('');
...
async function getSlotType(context, serviceMetadata, newSlotTypes, parameters) {
  const { inputs } = serviceMetadata;
  const { amplify, print } = context;
  const { executeProviderUtils } = amplify; 
  let slotType;
  ...
  if (slotTypeChoice[inputs[26].key] === 'Amazon built-in slot type') {
    ...
    let result;
    do {
      result = await executeProviderUtils(context, 'awscloudformation', 'getBuiltInSlotTypes', options);
      builtInSlotTypes = builtInSlotTypes.concat(result.slotTypes.map(builtinSlotType => builtinSlotType.signature));
      options = result.nextToken;
    } while (options);
    ...
    let slotTypes = await executeProviderUtils(context, 'awscloudformation', 'getSlotTypes');
...
async function askLambdaSource(context, functionType, fucntionPath) {
  switch (functionType) {
    case 'arn': return askLambdaArn(context);
    case 'projectFunction': return askLambdaFromProject(context);
    case 'newFunction': return newLambdaFunction(context, fucntionPath);
    default: throw new Error('Type not supported');
  }
}

function newLambdaFunction(context, fucntionPath) {
  let add;
  try {
