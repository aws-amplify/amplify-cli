...
function constructEnvNameInputPrompt(initialEnvName: string, isEnvNameValid: validatorFunction): InputPrompt {
...
export async function envNameInput(initialEnvName: string, isEnvNameValid: validatorFunction): Promise<string> {
  const answer = constructEnvNameInputPrompt(initialEnvName, isEnvNameValid).run();
  return answer;
}
