...
import { hashGQLResource } from 'amplify-provider-awscloudformation';
...
async function isBackendDirModifiedSinceLastPush(resourceName: string, categoryName: string, meta: $TSMeta) {
  /**
   * TODO function per category
   */
  let hashFunction: Function = getHashForResourceDir;
  if (categoryName === 'function' && meta[categoryName][resourceName].service === FunctionServiceName.LambdaLayer) {
    hashFunction = hashLayerResource;
  }
  if (categoryName === 'api' && meta[categoryName][resourceName].service === 'AppSync') {
    hashFunction = hashGQLResource;
  }

  // Pushing the resource for the first time hence no lastPushTimeStamp
  if (!meta[categoryName][resourceName].lastPushTimeStamp) {
    return false;
  }

  const localBackendDir = path.normalize(path.join(pathManager.getBackendDirPath(), categoryName, resourceName));
  const cloudBackendDir = path.normalize(path.join(pathManager.getCurrentCloudBackendDirPath(), categoryName, resourceName));
  ...
  await asyncForEach(Object.keys(amplifyMeta), async categoryName => {
    ...
    await asyncForEach(Object.keys(categoryItem), async resource => {
      ...
      if (
        ...
        const backendModified = await isBackendDirModifiedSinceLastPush(resource, categoryName, currentAmplifyMeta);
