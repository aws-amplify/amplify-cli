...
import { AuthParameters } from 'amplify-category-auth/lib/provider-utils/awscloudformation/import/types';
import { getFrontendConfig } from 'amplify-category-auth/lib/provider-utils/awscloudformation/utils/amplify-meta-updaters';
...
function ensureAmplifyMetaFrontendConfig(context, amplifyMeta?) {
  if (!amplifyMeta) {
  ...
  if (amplifyMeta.auth) {
    ...
    Object.keys(frontendAuthConfig).forEach(function (key) {
      if (!amplifyMeta.auth[authResourceName].frontendAuthConfig.hasOwnProperty(key)) {
        amplifyMeta.auth[authResourceName].frontendAuthConfig[key] = frontendAuthConfig[key];
      }
    });
