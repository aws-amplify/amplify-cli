...
export function displaySimpleString(context: Context, title: string,
  contents: string, indentation: number = 0) {
...
export function displayStringArray(context: Context, title: string,
  contents: string[], indentation: number = 0) {
  const indentationStr = createIndentation(indentation);
  context.print.blue(`${indentationStr}${title}:`);
...
export function displayPluginDirectories(context: Context, pluginPlatform: PluginPlatform) {
  context.print.info('');
  displayStringArray(context, 'Directories where the CLI scans for plugins:',
...
export function displayPrefixes(context: Context, pluginPlatform: PluginPlatform) {
  context.print.info('');
  displayStringArray(context, 'Plugin name prefixes for the CLI to search for:',
...
export function displayPluginCollection(context: Context,
  pluginCollection: PluginCollection, group: string = '') {
  Object.keys(pluginCollection).forEach((key) => {
    displayPluginInfoArray(context, pluginCollection[key], group);
  })
}

export function displayPluginInfoArray(context: Context,
  pluginInfoArray: Array<PluginInfo>, group: string = '') {
