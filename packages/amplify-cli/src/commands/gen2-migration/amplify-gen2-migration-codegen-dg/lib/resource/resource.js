...
function renderResourceTsFile({
  ...
  const exportStatement = factory.createVariableStatement(
    [factory.createModifier(typescript_1.default.SyntaxKind.ExportKeyword)],
    factory.createVariableDeclarationList([exportedVariable], typescript_1.default.NodeFlags.Const),
  );
  return factory.createNodeArray([
    ...importStatements,
    ...(postImportStatements !== undefined && postImportStatements.length > 0
      ? [ts_factory_utils_1.newLineIdentifier, ...postImportStatements]
      : []),
    ts_factory_utils_1.newLineIdentifier,
    exportStatement,
    ...(postExportStatements !== undefined && postExportStatements.length > 0
      ? [ts_factory_utils_1.newLineIdentifier, ...postExportStatements]
      : []),
  ]);
}
exports.renderResourceTsFile = renderResourceTsFile;
function renderResourceTsFilesForFunction({
  additionalImportedBackendIdentifiers = {},
  backendFunctionConstruct,
  functionCallParameter,
  exportedVariableName,
