...
import { assignProp } from '../../ts-factory-utils';
...
export class AuthGenerator implements Generator {
  ...
  private contributeToBackend(options: AuthRenderOptions): void {
    ...
    if (options.webClient?.AllowedOAuthFlows) {
      ...
      this.backendGenerator.addStatement(assignProp('cfnUserPoolClient', 'allowedOAuthFlows', options.webClient.AllowedOAuthFlows));
    }
