    ...
    for (const resource of customResources) {
      ...
      if (!backendContent.includes(importStatement)) {
        ...
        if (lastImportMatch) {
          ...
          backendContent = backendContent.slice(0, insertIndex) + 
                         importStatement + '\n' + 
...
`;
  ...
  private static findCustomResourceFiles(projectPath: string): string[] {
    ...
    return files.filter(file => 
      ...
      this.containsAmplifyHelpers(file)
    );
  }
  ...
  private static containsAmplifyHelpers(filePath: string): boolean {
    try {
      const content = fs.readFileSync(filePath, 'utf8');
      return content.includes('AmplifyHelpers') || 
             content.includes('@aws-amplify/cli-extensibility-helper');
    } catch {
      return false;
    }
  }
  ...
  private static handleResourceDependencies(newContent: string, originalContent: string): string {
    ...
    const addResourceDependencyRegex = /const\s+\w+:\s*AmplifyDependentResourcesAttributes\s*=\s*AmplifyHelpers\.addResourceDependency\([^;]*\);?\s*/g;
