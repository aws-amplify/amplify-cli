...
export class AmplifyHelperTransformer {
            ...
            if (ts.isPropertyAccessExpression(expression) &&
              ...
              if (methodName === 'getProjectInfo') {
                return ts.factory.createObjectLiteralExpression([
                  ts.factory.createPropertyAssignment(
                    ...
                    ts.factory.createPropertyAccessExpression(
                      ts.factory.createCallExpression(
                        ts.factory.createPropertyAccessExpression(
                          ts.factory.createIdentifier('Stack'),
                  ...
                  ts.factory.createPropertyAssignment(
                    'envName',
                    ts.factory.createIdentifier('branchName')
                  )
                ]);
              }
  ...
  static addRequiredImports(sourceFile: ts.SourceFile): ts.SourceFile {
    ...
    const hasStackImport = sourceFile.statements.some(stmt =>
      ts.isImportDeclaration(stmt) &&
      ts.isStringLiteral(stmt.moduleSpecifier) &&
      stmt.moduleSpecifier.text === 'aws-cdk-lib' &&
      stmt.importClause?.namedBindings &&
      ts.isNamedImports(stmt.importClause.namedBindings) &&
      stmt.importClause.namedBindings.elements.some(element => 
        element.name.text === 'Stack'
      )
    );
    ...
    if (!hasStackImport) {
      ...
      return ts.factory.updateSourceFile(sourceFile, [
        stackImport,
        ...sourceFile.statements.filter(stmt => 
          !(ts.isImportDeclaration(stmt) &&
            ts.isStringLiteral(stmt.moduleSpecifier) &&
