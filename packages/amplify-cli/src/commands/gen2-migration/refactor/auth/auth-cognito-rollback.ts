...
export class AuthCognitoRollbackRefactorer extends RollbackCategoryRefactorer {
  ...
  protected override async afterMove(gen2StackId: string): Promise<AmplifyMigrationOperation[]> {
    ...
    const userPoolId = await this.gen2Branch.fetchUserPoolId(holdingStackName);
    if (!userPoolId) {
      throw new AmplifyError('MigrationError', {
        message: `Unable to discover UserPool in holding stack '${holdingStackName}' for social auth re-import`,
      });
    }
