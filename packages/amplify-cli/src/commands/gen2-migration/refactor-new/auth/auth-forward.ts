 ...
 * main auth (UserPool, UserPoolClient, IdentityPool, etc.) and UserPoolGroups.
 * Gen2 combines them into one stack.
 *
...
export class AuthForwardRefactorer extends ForwardCategoryRefactorer {
  ...
  public override async plan(): Promise<RefactorOperation[]> {
    ...
    // 4. One shared beforeMove (holding stack for ALL Gen2 auth resources)
    const { operations: beforeMoveOps, postTargetTemplate } = this.beforeMovePlan(mainAuthSource, target);

    // 5. Chain buildRefactorTemplates: second move uses output of first
    ...
    // 6. UserPoolGroup move (if exists) — chains from gen2AfterMainAuth
    ...
    // 7. Assemble operations in order
    ...
    // afterMove is empty for forward
    return ops;
  }

  /**
   * OAuth hook: retrieves OAuth credentials and updates the hostedUIProviderCreds parameter.
   */
  protected override async resolveOAuthParameters(parameters: Parameter[], outputs: Output[]): Promise<Parameter[]> {
    ...
    if (!credsParam) {
      throw new AmplifyError('MissingExpectedParameterError', {
        message: `Auth stack parameter '${HOSTED_PROVIDER_CREDENTIALS_PARAMETER_NAME}' not found — required for OAuth credential injection`,
      });
    }
   ...
   *
   * For types with single instances: match by type.
   * For UserPoolClient: Gen1 UserPoolClientWeb → Gen2's non-Native client;
   *   Gen1's other client → Gen2's *UserPoolNativeAppClient*.
   * For UserPoolGroup/IAM Role: Gen2 logical ID contains Gen1 logical ID.
   */
    ...
    for (const [sourceId, sourceResource] of sourceResources) {
      for (const [targetId, targetResource] of targetResources) {
        ...
        if (TYPES_WITH_MULTIPLE_RESOURCES.includes(sourceResource.Type)) {
          // UserPoolClient: Web↔Web, Native↔Native
          ...
          // UserPoolGroup / IAM Role: Gen2 ID contains Gen1 ID
