...
function cfnChangesetConsoleUrl(changeSetArn: string, stackArn?: string): string | undefined {
  ...
  return `https://${region}.console.aws.amazon.com/cloudformation/home?region=${region}#/stacks/${encodedStackId}/changesets/${encodedChangeSetId}/changes`;
}
    ...
    const flattenChanges = (changes: ResourceChangeWithNested[], fallbackCategory: string, fallbackChangeSetId?: string, fallbackStackId?: string): void => {
      for (const change of changes) {
        if (change.ResourceType === 'AWS::CloudFormation::Stack' && change.nestedChanges && change.nestedChanges.length > 0) {
          flattenChanges(change.nestedChanges, extractCategory(change.LogicalResourceId), change.ChangeSetId || fallbackChangeSetId, change.PhysicalResourceId || fallbackStackId);
        } else {
          ...
          blocks.push({
            ...
            stackId: change.PhysicalResourceId || fallbackStackId,
          });
        }
      }
    };
...
function formatBlock(block: DriftBlock): string {
  ...
  if (block.type === 'cf') {
    ...
    if (block.changeSetId) {
      const changesetUrl = cfnChangesetConsoleUrl(block.changeSetId, block.stackId);
