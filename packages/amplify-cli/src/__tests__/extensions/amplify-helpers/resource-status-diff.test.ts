import { capitalize, 
         globCFNFilePath, 
         ResourceDiff, 
         stackMutationType} from '../../../extensions/amplify-helpers/resource-status-diff';
...
const allFiles: string[] =  [ "cloudformation-template.json",	"parameters.json" , "resolvers", 
                              "stacks" ,"functions", "pipelinesFunctions", "schema.graphql" ]
...
describe('resource-status-diff helpers', () => {
    ...
    it('should search both Build and non Build folders for Cloudformation Templates ', async () => {
        ...
        const checkCfnPaths = ()=> {
            const mockLocalPreBuildCfnFile = path.join(localBackendDirPathStub, input.mockCategory, input.mockResourceName, input.mockDefaultRootCfnTmpltName);
            const mockCurrentPreBuildCfnFile = path.join(currentBackendDirPathStub, input.mockCategory, input.mockResourceName, input.mockDefaultRootCfnTmpltName);
            ...
            const mockCurrentPostBuildCfnFile = path.join(currentBackendDirPathStub,input.mockCategory, input.mockResourceName,'build', input.mockDefaultRootCfnTmpltName);
