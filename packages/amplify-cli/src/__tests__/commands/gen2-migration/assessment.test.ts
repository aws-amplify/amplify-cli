...
describe('Assessment', () => {
  describe('record()', () => {
    it('creates an entry on first record for a resource', () => {
      ...
      const resource: DiscoveredResource = { category: 'auth', resourceName: 'myPool', service: 'Cognito' };
    ...
    it('updates an existing entry without overwriting the other step', () => {
      const assessment = new Assessment('app', 'dev');
      const resource: DiscoveredResource = { category: 'storage', resourceName: 'myBucket', service: 'S3' };
    ...
    it('records notes correctly', () => {
      const assessment = new Assessment('app', 'dev');
      const resource: DiscoveredResource = { category: 'function', resourceName: 'myFunc', service: 'Lambda' };
    ...
    it('handles multiple resources across categories', () => {
      ...
      assessment.record('generate', { category: 'auth', resourceName: 'pool', service: 'Cognito' }, { supported: true, notes: [] });
      assessment.record('generate', { category: 'storage', resourceName: 'bucket', service: 'S3' }, { supported: true, notes: [] });
      assessment.record('generate', { category: 'geo', resourceName: 'map', service: 'Location' }, { supported: false, notes: [] });
