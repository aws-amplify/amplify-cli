...
describe('CategoryRefactorer.plan() orchestration — via StorageS3ForwardRefactorer', () => {
  ...
  it('throws when both stacks are absent (Path A)', async () => {
    ...
    await expect(new StorageS3ForwardRefactorer(gen1Env, gen2Branch, clients, 'us-east-1', '123', null as any).plan()).rejects.toThrow(
      'unable to find source stack',
    );
  });
  ...
  it('returns empty array when no matching resource types in source (Path D)', async () => {
    ...
    const { clients, gen1Env, gen2Branch } = makeInstances();
    const ops = await new StorageS3ForwardRefactorer(gen1Env, gen2Branch, clients, 'us-east-1', '123', null as any).plan();
    expect(ops).toEqual([]);
  });
...
describe('StorageS3RollbackRefactorer.plan() — rollback without holding stack', () => {
  ...
  it('produces move only (no updateSource/updateTarget, no afterMove ops)', async () => {
    ...
    const { clients, gen1Env, gen2Branch } = makeInstances();
    const ops = await new StorageS3RollbackRefactorer(gen1Env, gen2Branch, clients, 'us-east-1', '123', null as any).plan();
    const descriptions = (await Promise.all(ops.map((o) => o.describe()))).flat();
...
describe('Analytics wiring tests', () => {
  ...
  it('forward: discovers analytics stacks and maps Kinesis stream', async () => {
    ...
    const { clients, gen1Env, gen2Branch } = makeInstances();
    const ops = await new AnalyticsKinesisForwardRefactorer(gen1Env, gen2Branch, clients, 'us-east-1', '123', null as any).plan();
    const descriptions = (await Promise.all(ops.map((o) => o.describe()))).flat();
  ...
  it('rollback: discovers analytics stacks and maps to Gen1 KinesisStream ID', async () => {
    ...
    setupAnalyticsMocks(cfnMock);
    const { clients, gen1Env, gen2Branch } = makeInstances();
    const ops = await new AnalyticsKinesisRollbackRefactorer(gen1Env, gen2Branch, clients, 'us-east-1', '123', null as any).plan();
    const descriptions = (await Promise.all(ops.map((o) => o.describe()))).flat();
