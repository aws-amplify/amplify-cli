...
describe('amplify authUI test', () => {
  ...
  describe('Run test on JS app:', async () => {
    ...
    it('should set up amplify backend and generate aws-export.js file', async () => {
      ...
      //projRoot = '/Users/lizeyu/Desktop/workspace/photo-albums'
    });
    ...
    it('should have user pool in backend and sign up a user for test', async () => {
      ...
      const settings = {
        username: 'test01',
        password: 'The#test1',
        email: 'samlmar+test01@amazon.com',
        clientId: clientId,
    ...
    it.skip('should pass all UI tests on Angular app', async () => {
      ...
      await runCypressTest(destRoot, {platform: 'angular'}).then(isPassed => expect(isPassed).toBeTruthy())
    })
    ...
    it.skip('should pass all UI tests on Vue app', async () => {
      ...
      await runCypressTest(destRoot, {platform: 'vue'}).then(isPassed => expect(isPassed).toBeTruthy())
    });
