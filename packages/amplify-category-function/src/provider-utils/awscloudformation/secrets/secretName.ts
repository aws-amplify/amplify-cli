 ...
 * This format is standardized with other Amplify Console SSM parameters and is expected by customer functions when fetching secrets at
 * runtime
 */
 ...
 */
export const getFullyQualifiedSecretName = (
  secretName: string,
  functionName: string,
  envName?: string,
): string => `${getFunctionSecretPrefix(functionName, envName)}${secretName}`;
 ...
 * If envName is not specified, the current env is assumed
 */
export const getFunctionSecretPrefix = (
  functionName: string,
  envName?: string,
): string => path.posix.join(getEnvSecretPrefix(envName), `AMPLIFY_${functionName}_`);
 ...
 * If envName is not specified, the current env is assumed
 */
export const getEnvSecretPrefix = (envName: string = stateManager.getLocalEnvInfo()?.envName): string => {
  if (!envName) {
...
// NOTE: Even though the following 2 functions are CFN specific, I'm putting them here to co-locate all of the secret naming logic

/**
 * Returns a CFN object that will resolve the the full name of the SSM parameter for secretName in functionName
 */
export const getFunctionSecretCfnName = (secretName: string, functionName: string): IntrinsicFunction => Fn.Join('', [getFunctionSecretCfnPrefix(functionName), secretName]);

/**
 * Returns a CFN object that will resolve to the SSM parameter name prefix for all secrets in a given function
 */
export const getFunctionSecretCfnPrefix = (functionName: string): IntrinsicFunction => Fn.Sub(path.posix.join('/amplify', '${appId}', '${env}', 'AMPLIFY_${functionName}_'), {
...
/**
 * Get the Amplify App ID from the current environment metadata
 */
