 ...
 */
export const askEnvironmentVariableCarryOut = async (context: $TSContext, fromEnvName: string, yesFlagSet?: boolean): Promise<void> => {
  ...
  const envVarQuestion = async (): Promise<void> => {
    ...
    const envVar = yesFlagSet
      ? 'carry'
      : await prompter.pick<'one', string>('You have configured environment variables for functions. How do you want to proceed?', [
          {
            value: 'carry',
  ...
  const envVarSelectFunction = async (): Promise<void> => {
    ...
    const functionName = await prompter.pick<'one', string>('Select the Lambda function you want to update values', choices);
  ...
  const envVarSelectKey = async (functionName: string): Promise<void> => {
    ...
    const keyName = await prompter.pick<'one', string>("Which function's environment variables do you want to edit?", choices);
    if (keyName === abortKey) {
