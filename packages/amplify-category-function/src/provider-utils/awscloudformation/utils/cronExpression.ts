...
export class CronExpression{
    ...
    protected storeExpressionVals  = function(pos : number , s : string , type : number) : number{
            ...
            if (type === MONTH) {
                ...
                if (s.length > i + 3) {
                    ...
                    if (c === '-') {
                        ...
                        if (sVal > eVal) {
                            throw new Error(
                                    "Invalid Day-of-Week sequence: " + String(sVal)
                                            + " > " + String(eval) +  String(i));
                        }
                    }
                    else if (c === '#') {
                        try {
                ...
                throw new Error(
                        "Illegal characters for this position: '" + String(sub) + "'" +
                        String(i));
            }
            if (eVal != -1) {
                incr = 1;
        ...
        if (c === '?') {
            ...
            if ((i + 1) < s.length
                ...
                throw new Error("Illegal character after '?': "
                        + s.charAt(i) + String(i));
            }
            ...
            if (c === '*' && (i + 1) >= s.length) {
                ...
                return i + 1;
                    ...
                    && ((i + 1) >= s.length || s.charAt(i + 1) === ' ' || s
                            .charAt(i + 1) === '\t')) {
                throw new Error("'/' must be followed by an integer." +
                        String(i));
            }
            else if (c === '*') {
                i++;
    ...
    protected  addToSet = function(val : number, end : number, incr : number, type : number) {
        ...
        if (type === SECOND || type === MINUTE) {
            ...
            if ((val < 1 || val > 31 || end > 31) && (val != ALL_SPEC_INT)
                    && (val != NO_SPEC_INT)) {
                throw new Error(
                        "Day of month values must be between 1 and 31" + String(-1));
            }
        } else if (type === MONTH) {
            if ((val < 1 || val > 12 || end > 12) && (val != ALL_SPEC_INT)) {
                throw new Error(
                        "Month values must be between 1 and 12" + String(-1));
            }
        } else if (type === DAY_OF_WEEK) {
            if ((val === 0 || val > 7 || end > 7) && (val != ALL_SPEC_INT)
                    && (val != NO_SPEC_INT)) {
                throw new Error(
                        "Day-of-Week values must be between 1 and 7" + String(-1));
            }
        }
    ...
    if (c === 'L') {
        if (type === DAY_OF_WEEK) {
            ...
            throw new Error("'L' option is not valid here. (pos="
                    + i + ")" +  i);
        }
        let set = this.getSet(type);
        set.add(val);
    ...
    if (c === 'W') {
        if (type === DAY_OF_MONTH) {
            ...
            throw new Error("'W' option is not valid here. (pos="
                    + i + ")" + i);
        }
        let set = this.getSet(type);
        set.add(val);
    ...
    if (c === '#') {
        if (type != DAY_OF_WEEK) {
            throw new Error("'#' option is not valid here. (pos="
                    + i + ")" +  i);
        }
        i++;
        try {
