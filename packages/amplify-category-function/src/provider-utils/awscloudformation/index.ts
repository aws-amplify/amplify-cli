...
import { MergeableFunctionParameters } from './MergeableFunctionParameters';
 ...
 */
export async function addResource(context, category, service, options, parameters?: FunctionParameters | FunctionTriggerParameters): Promise<string> {
  // load the service config for this service
  const serviceConfig: ServiceConfig = SUPPORTED_SERVICES[service]
  if (!serviceConfig) {
    throw `amplify-category-function is not configured to provide service type ${service}`;
  }

  // if the parameters are a trigger, skip the walkthrough
  if (!parameters || !('trigger' in parameters)) {
    // initialize the template parameters
    const funcParams = new MergeableFunctionParameters({
      provider: provider,
      service: serviceName,
    ...
    funcParams.merge(parameters);

    // merge in the CFN file
    funcParams.merge({ cloudResourceTemplatePath: serviceConfig.cfnFilename });
  ...
  copyFunctionResources(context, parameters)

  if (!parameters.skipEdit) {
    await openEditor(context, category, parameters);
  }
...
async function openEditor(context, category, options: FunctionParameters | FunctionTriggerParameters) {
  ...
  if (await context.amplify.confirmPrompt.run(`Do you want to edit the ${displayName} lambda function now?`)) {
    ...
    const target = path.join(targetDir, category, options.resourceName, 'src', targetFile);
...
export async function updateConfigOnEnvInit(context, category, service) {
  const srvcMetaData: ServiceConfig = SUPPORTED_SERVICES.Lambda; // FIXME this shouldn't be hardcoded to lambda!
