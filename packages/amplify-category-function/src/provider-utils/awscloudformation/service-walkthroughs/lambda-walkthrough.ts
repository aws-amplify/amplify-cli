...
export const createWalkthrough = async (
  ...
  advancedSettingsList.forEach(setting => printer.info('- '.concat(setting)));
  printer.info('');
...
const provideInformation = (context, lambdaToUpdate, functionRuntime, currentParameters, scheduleParameters): void => {
  ...
  printer.info('- Name: '.concat(lambdaToUpdate));
  printer.info('- Runtime: '.concat(functionRuntime));
  printer.info('');

  // Provide resource access permission information
  printer.success('Resource access permission');
  ...
  if (currentCategoryPermissions.length) {
    currentCategoryPermissions.forEach(category => {
      ...
      currentResources.forEach(resource => {
        ...
        printer.info('- '.concat(resource).concat(formattedCurrentPermissions));
      });
    });
  } else {
    printer.info('- Not configured');
  }
  printer.info('');

  // Provide scheduling information
  printer.success('Scheduled recurring invocation');
  // eslint-disable-next-line spellcheck/spell-checker
  if (scheduleParameters.cloudwatchRule && scheduleParameters.cloudwatchRule !== 'NONE') {
    // eslint-disable-next-line spellcheck/spell-checker
    printer.info('- '.concat(scheduleParameters.cloudwatchRule));
    printer.info('');
  } else {
    printer.info('- Not configured');
    printer.info('');
  ...
  printer.success('Environment variables:');
  ...
  if (_.size(storedEnvironmentVariables) !== 0) {
    _.forEach(storedEnvironmentVariables, (environmentVariableValue, environmentVariableKey) => {
      printer.info(`- ${environmentVariableKey}: ${environmentVariableValue}`);
    });
  } else {
    printer.info('- Not configured');
  }
  ...
  printer.success('Secrets configuration');
  const currentSecrets = getLocalFunctionSecretNames(lambdaToUpdate);
  if (currentSecrets.length) {
    currentSecrets.forEach(secretName => printer.info(`- ${secretName}`));
  } else {
    printer.info('- Not configured');
  }
  printer.info('');
...
export const updateWalkthrough = async (context: $TSContext, lambdaToUpdate?: string): Promise<Partial<FunctionParameters>> => {
  ...
  if (lambdaFuncResourceNames.length === 0) {
    printer.error('No Lambda function resource to update. Use "amplify add function" to create a new function.');
    return undefined;
  }

  if (lambdaToUpdate) {
    if (!lambdaFuncResourceNames.includes(lambdaToUpdate)) {
      printer.error(`No Lambda function named ${lambdaToUpdate} exists in the project.`);
      return undefined;
    }
  } else {
    lambdaToUpdate = await prompter.pick('Select the Lambda function you want to update', lambdaFuncResourceNames);
  }
  ...
  const selectedSettings = await settingsUpdateSelection();
