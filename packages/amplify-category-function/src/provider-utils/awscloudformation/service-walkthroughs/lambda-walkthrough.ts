 ...
 */
export async function createWalkthrough(context: any, templateParameters: MergeableFunctionParameters): Promise<FunctionParameters> {
  // merge in parameters that don't require any additional input
  templateParameters.merge(autogeneratedParameters(context));

  // ask generic function questions and merge in results
  templateParameters.merge(await generalQuestionsWalkthrough(context));

  // ask runtime selection questions and merge in results
  if (!templateParameters.runtime) {
    templateParameters.merge(await runtimeWalkthrough(context, templateParameters));
  }

  // ask template selection questions and merge in results
  templateParameters.merge(await templateWalkthrough(context, templateParameters));
