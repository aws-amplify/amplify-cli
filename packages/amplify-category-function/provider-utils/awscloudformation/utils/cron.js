    ...
    switch(intervalAnswer.interval){
        ...
        case 'weekly' :{
            const WeekQuestion = {
                type: 'list',
                name: 'week',
                message: 'Please select the  da to start Job?',
                choices: [ '1','2', '3','4','5','6','7'],
            };
            var exp1 = new cb();
            const weekAnswer = await inquirer.prompt([WeekQuestion]);
            exp1.set('minute', timeAnswer.dt.getMinutes().toString().split());
...
function makeCron(interval,dateAnswer,timeAnswer){
    var cronExp = new cb();
    if(interval === 'monthly'){
        cronExp.set('minute', timeAnswer.dt.getMinutes().toString().split());
        cronExp.set('hour', timeAnswer.dt.getHours().toString().split());
        cronExp.set('dayOfTheMonth', dateAnswer.dt.getDate().toString().split());
    }
        ...
        cronExp.set('hour', timeAnswer.dt.getHours().toString().split());
        cronExp.set('dayOfTheMonth', dateAnswer.dt.getDate().toString().split());
        cronExp.set('month', dateAnswer.dt.getMonth().toString().split());
    }
