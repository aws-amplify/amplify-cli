...
class BackendSynthesizer {
    ...
    createUserPoolClientAssignment(userPoolClient, imports) {
        ...
        const userPoolClientDeclaration = factory.createVariableStatement(undefined, factory.createVariableDeclarationList([
            factory.createVariableDeclaration(factory.createIdentifier('userPoolClient'), undefined, undefined, factory.createCallExpression(factory.createPropertyAccessExpression(factory.createIdentifier('userPool'), factory.createIdentifier('addClient')), undefined, [
    ...
    createAmplifyEnvNameLogic() {
        ...
        factory.createLogicalAnd(factory.createPropertyAccessExpression(factory.createIdentifier('ci'), factory.createIdentifier('isCI')), factory.createLogicalNot(factory.createIdentifier('AMPLIFY_GEN_1_ENV_NAME'))), 
        ...
        factory.createIfStatement(factory.createLogicalAnd(factory.createLogicalNot(factory.createPropertyAccessExpression(factory.createIdentifier('ci'), factory.createIdentifier('isCI'))), factory.createLogicalNot(factory.createIdentifier('AMPLIFY_GEN_1_ENV_NAME'))), 
    ...
    render(renderArgs) {
        ...
        if (renderArgs.auth || renderArgs.storage?.hasS3Bucket || renderArgs.customResources) {
            imports.push(this.createImportStatement([factory.createIdentifier('RemovalPolicy'), factory.createIdentifier('Tags')], 'aws-cdk-lib'));
        }
        if (renderArgs.auth) {
        ...
        if (renderArgs.storage?.dynamoDB) {
            nodes.push(factory.createThrowStatement(factory.createNewExpression(factory.createIdentifier('Error'), undefined, [
                factory.createStringLiteral(`DynamoDB table \`${renderArgs.storage.dynamoDB}\` is referenced in your Gen 1 backend and will need to be manually migrated to reference with CDK.`),
            ])));
        }
        ...
        if (renderArgs.customResources) {
            for (const [resourceName, className] of renderArgs.customResources) {
                ...
                ])), factory.createStringLiteral(`./custom/${resourceName}/cdk-stack`), undefined);
        ...
        const ciInfoImportStatement = factory.createImportDeclaration(undefined, factory.createImportClause(false, factory.createIdentifier('ci'), undefined), factory.createStringLiteral('ci-info'));
        ...
        if (renderArgs.auth?.userPoolOverrides && !renderArgs?.auth?.referenceAuth) {
            ...
            for (const [overridePath, value] of Object.entries(renderArgs.auth.userPoolOverrides)) {
                if (overridePath.includes('userPoolName')) {
                    ...
                    const userPoolAssignment = this.createTemplateLiteralExpression('cfnUserPool', 'userPoolName', `${userPoolWithoutBackendEnvName}-`, amplifyGen1EnvName, '');
        ...
        if (renderArgs.auth?.guestLogin === false || (renderArgs.auth?.identityPoolName && !renderArgs?.auth?.referenceAuth)) {
            ...
            if (renderArgs.auth?.identityPoolName) {
                ...
                const identityPoolAssignment = this.createTemplateLiteralExpression('cfnIdentityPool', 'identityPoolName', `${identityPoolWithoutBackendEnvName}_`, amplifyGen1EnvName, '');
        ...
        if (renderArgs.storage && renderArgs.storage.hasS3Bucket) {
            ...
            const bucketNameAssignment = this.createTemplateLiteralExpression('// s3Bucket', 'bucketName', `${bucketNameWithoutBackendEnvName}-`, amplifyGen1EnvName, '');
        ...
        if (renderArgs.storage?.accelerateConfiguration ||
            ...
            imports.push(factory.createImportDeclaration(undefined, factory.createImportClause(false, undefined, factory.createNamespaceImport(factory.createIdentifier('s3'))), factory.createStringLiteral('aws-cdk-lib/aws-s3')));
        }
        if (renderArgs.auth?.userPoolClient &&
        ...
        if (renderArgs.auth || renderArgs.storage?.hasS3Bucket || renderArgs.customResources) {
            const tagAssignment = factory.createExpressionStatement(factory.createCallExpression(factory.createPropertyAccessExpression(factory.createCallExpression(factory.createIdentifier('// Tags.of'), undefined, [factory.createIdentifier('backend.stack')]), factory.createIdentifier('add')), undefined, [factory.createStringLiteral('gen1-migrated-app'), factory.createStringLiteral('true')]));
