 ...
 */
const createGen2Renderer = ({ outputDir, backendEnvironmentName, auth, storage, data, functions, customResources, unsupportedCategories, fileWriter = (content, path) => createFileWriter(path)(content), }) => {
    ...
    const amplifyPackageJson = new package_json_1.JsonRenderer(async () => ({ type: 'module' }), (content) => fileWriter(content, path_1.default.join(outputDir, 'amplify', 'package.json')));
        ...
        for (const func of functions) {
            if (func.name) {
                ...
                renderers.push(new typescript_block_node_1.TypescriptNodeArrayRenderer(async () => (0, index_3.renderFunctions)(func), (content) => {
                    ...
                    return fileWriter(content, path_1.default.join(dirPath, 'resource.ts')).then(() => fileWriter('', path_1.default.join(dirPath, 'handler.ts')));
                }));
            }
        }
    ...
    if (data && data.tableMappings && backendEnvironmentName && data.tableMappings[backendEnvironmentName] !== undefined) {
        ...
        renderers.push(new typescript_block_node_1.TypescriptNodeArrayRenderer(async () => (0, index_2.generateDataSource)(data), (content) => fileWriter(content, path_1.default.join(outputDir, 'amplify', 'data', 'resource.ts'))));
    ...
    if (storage) {
        ...
        if (hasS3Bucket) {
            ...
            renderers.push(new typescript_block_node_1.TypescriptNodeArrayRenderer(async () => (0, index_js_1.renderStorage)(storage), (content) => fileWriter(content, path_1.default.join(outputDir, 'amplify', 'storage', 'resource.ts'))));
        }
