...
function renderFunctions(definition, appId, backendEnvironmentName) {
    ...
    postImportStatements.push(factory.createExpressionStatement(factory.createCallExpression(factory.createIdentifier('throw new Error'), undefined, [
        factory.createStringLiteral(`Source code for this function can be found in your Amplify Gen 1 Directory. See .amplify/migration/amplify/backend/function/${definition.resourceName}/src`),
    ])));
    ...
    if (definition?.environment?.Variables) {
        defineFunctionProperties.push(createParameter('environment', factory.createObjectLiteralExpression(Object.entries(definition.environment.Variables).map(([key, value]) => {
            if (key == 'API_KEY' && value.startsWith(`/amplify/${appId}/${backendEnvironmentName}`)) {
                postImportStatements?.push(factory.createCallExpression(factory.createIdentifier('throw new Error'), undefined, [
