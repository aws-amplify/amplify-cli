...
const generateDataSource = (dataDefinition) => {
    ...
    if (dataDefinition && dataDefinition.schema) {
        const schemaVariableDeclaration = factory.createVariableDeclaration('schema', undefined, undefined, factory.createNoSubstitutionTemplateLiteral(dataDefinition.schema));
    ...
    if (dataDefinition?.tableMappings) {
        ...
        for (const [environmentName, tableMapping] of Object.entries(dataDefinition.tableMappings)) {
            ...
            const branchNameExpression = typescript_1.default.addSyntheticLeadingComment(factory.createPropertyAssignment('branchName', factory.createStringLiteral(environmentName)), typescript_1.default.SyntaxKind.SingleLineCommentTrivia, ` Replace the environment name (${environmentName}) with the corresponding branch name. Use "sandbox" for your sandbox environment.`, true);
            let tableMappingExpression = factory.createPropertyAssignment('modelNameToTableNameMapping', factory.createObjectLiteralExpression(tableMappingProperties));
            if (tableMappingProperties.length === 0) {
                tableMappingExpression = typescript_1.default.addSyntheticLeadingComment(tableMappingExpression, typescript_1.default.SyntaxKind.MultiLineCommentTrivia, '*\n' +
        ...
        dataRenderProperties.push(factory.createPropertyAssignment(migratedAmplifyGen1DynamoDbTableMappingsKeyName, factory.createArrayLiteralExpression(tableMappingEnvironments)));
    }
