...
import { getCLIPath, getScriptRunnerPath } from '../util';
...
export function addEnvironment(cwd: string, settings: any): Promise<void> {
  return new Promise((resolve, reject) => {
    spawn(getScriptRunnerPath(), [getCLIPath(), 'env', 'add', '--providers', JSON.stringify(settings.providersParam)], { cwd, stripColors: true })
...
export function amplifyPublish(cwd: string): Promise<void> {
  return new Promise((resolve, reject) => {
    spawn(getScriptRunnerPath(), [getCLIPath(), 'publish'], { cwd, stripColors: true })
      .wait('Are you sure you want to continue?')
      .sendLine('\r')
      .run((err: Error) => {
        if (!err) {
...
export function amplifyConfigure(cwd: string): Promise<void> {
  return new Promise((resolve, reject) => {
    spawn(getScriptRunnerPath(), [getCLIPath(), 'hosting', 'configure'], { cwd, stripColors: true })
...
export function amplifyStatus(cwd: string, expectedStatus: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    spawn(getScriptRunnerPath(), [getCLIPath(), 'status'], { cwd, stripColors: true })
      .wait(regex)
      .sendLine('\r')
      .run((err: Error) => {
        if (!err) {
...
export function checkoutEnv(cwd: string, env: string): Promise<void> {
  return new Promise((resolve, reject) => {
    spawn(getScriptRunnerPath(), [getCLIPath(), 'env', 'checkout', env], { cwd, stripColors: true }).run((err: Error) => {
      if (!err) {
        resolve();
      } else {
        reject(err);
