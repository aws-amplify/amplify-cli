...
test('validatePathName_invalidCharacters', () => {
    // setup
    const errorMessage = 'Each path part must use characters a-z A-Z 0-9 - and must not be empty.\nOptionally, a path part can be surrounded by { } to denote a path parameter.';
...
test('validatePathName_pathMatch', () => {
    expect(validatePathName(stubOtherPaths[0].name, stubOtherPaths)).toStrictEqual('An existing path already matches this sub-path: /other/path')
});
