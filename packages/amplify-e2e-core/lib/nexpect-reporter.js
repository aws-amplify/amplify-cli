    ...
    if (ancestorTitles.length) {
        ...
        if (describeBlock) {
            ...
            if (describeBlock.hooks.beforeAll) {
                before = getLogs(describeBlock.hooks.beforeAll).map((bfa) => (Object.assign(Object.assign({}, bfa), { name: `${prefixStr} -> BeforeAll` })));
            }
            children = mergeCliLog(result, describeBlock.children, ancestorTitles.slice(1), prefixStr);
            if (describeBlock.hooks.afterAll) {
                after = getLogs(describeBlock.hooks.afterAll).map((afa) => (Object.assign(Object.assign({}, afa), { name: `${prefixStr} --> AfterAll` })));
            }
        }
    }
    else {
        ...
        if (testBlock) {
            ...
            if (testBlock.hooks.beforeEach) {
                before = getLogs(testBlock.hooks.beforeEach).map((bfe) => (Object.assign(Object.assign({}, bfe), { name: `${prefixStr} -> BeforeEach` })));
            }
            if (filterBlock([testBlock]).length) {
                children = testBlock.logs.map((log) => (Object.assign(Object.assign({}, log), { name: prefixStr })));
            }
            if (testBlock.hooks.afterEach) {
                after = getLogs(testBlock.hooks.afterEach).map((afe) => (Object.assign(Object.assign({}, afe), { name: `${prefixStr} --> AfterEach` })));
            }
        }
    }
...
class AmplifyCLIExecutionReporter {
    ...
    onRunComplete(contexts, results) {
        ...
        const processedResults = results.testResults.map((result) => {
            ...
            return Object.assign(Object.assign({}, resultCopy), { 
                ...
                testResults: result.testResults.map((r) => {
                    ...
                    const recordingWithPath = recordings.map((r, index) => {
                        ...
                        return Object.assign(Object.assign({}, rCopy), { castFile: `./${castFile}` });
                    });
                    return Object.assign(Object.assign({}, r), { recordings: recordingWithPath });
                }) });
        });
        const resultsWithRecordings = Object.assign(Object.assign({}, results), { testResults: processedResults });
        resultsWithRecordings.config = this._globalConfig;
        resultsWithRecordings.endTime = Date.now();
        resultsWithRecordings._reporterOptions = Object.assign(Object.assign({}, this._options), { logoImg, customInfos: {} });
...
//# sourceMappingURL=nexpect-reporter.js.map