...
function generateFilterExpression(filter, prefix = null, parent = null) {
    const expr = Object.entries(filter).reduce((sum, [name, value]) => {
        let subExpr = {
            expressions: [],
            expressionNames: {},
            expressionValues: {},
        ...
        switch (name) {
            ...
            case 'and': {
                ...
                if (Array.isArray(value)) {
                    subExpr = scopeExpression(value.reduce((expr, subFilter, idx) => {
...
function createExpressionValue(fieldName, op, value, prefix) {
    ...
    const exprValue = dynamodb_utils_1.dynamodbUtils.toDynamoDB(value);
    return {
        [`${exprName}`]: exprValue,
    };
}
