...
export class JavaString {
  ...
  split(regexString, limit = undefined) {
    ...
    const groupNum = ''.match(testRe).length; // actually num of groups plus one, ie "" and the (empty) groups

    const re = new RegExp(regexString.toString());

    const result = this.value.split(re, limit).filter((v, ii) => !(ii % groupNum));
