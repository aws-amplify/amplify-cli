 ...
 */
export const addLegacySmsNotificationChannel = async (cwd: string, resourceName: string, hasAnalytics = false): Promise<void> => {
 ...
 */
export const removeLegacyAllNotificationChannel = async (cwd: string): Promise<void> =>
  spawn(getCLIPath(false), ['remove', 'notifications'], { cwd, stripColors: true })
