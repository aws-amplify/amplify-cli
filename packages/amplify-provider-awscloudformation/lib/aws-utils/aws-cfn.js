...
const CFN_POLL_TIME = (process.env.IS_AMPLIFY_CI ? 30 : 5) * 1000;
const CFN_POLL_TIME_MAX = (process.env.IS_AMPLIFY_CI ? 120 : 30) * 1000;
let CFNLOG = [];
...
class CloudFormation {
        ...
        return new Promise((resolve, reject) => {
            ...
            cfnModel.createStack(cfnParentStackParams, (createErr) => {
                ...
                cfnModel.waitFor(cfnCompleteStatus, cfnStackCheckParams, async (completeErr, waitForStackdata) => {
                    ...
                    (_a = this.progressBar) === null || _a === void 0 ? void 0 : _a.stop();
                    if (completeErr) {
                        context.print.error('\nAn error occurred when creating the CloudFormation stack');
                        ...
                        const error = new AmplifyFault('DeploymentFault', { message: 'Initialization of project failed', details: errorDetails }, completeErr);
                        error.stack = null;
                        reject(error);
                    }
    ...
    filterFailedStackEvents(eventsWithFailure) {
        return eventsWithFailure
            ...
            .filter((stack) => (this.eventMap.logicalResourceNames && this.eventMap.logicalResourceNames.includes(stack.LogicalResourceId)) ||
    ...
    async updateamplifyMetaFileWithStackOutputs(parentStackName) {
        ...
        const stackSummaries = await pagedAWSCall(async (params, nextToken) => {
            return await this.cfn.listStackResources({ ...params, NextToken: nextToken }).promise();
        }, {
            StackName: parentStackName,
        }, (response) => response.StackResourceSummaries, async (response) => response.NextToken);
        const resources = stackSummaries.filter((resource) => ![
            'DeploymentBucket',
            'AuthRole',
            'UnauthRole',
        ...
        const rootStackResources = stackSummaries.filter((resource) => !['UpdateRolesWithIDPFunction', 'UpdateRolesWithIDPFunctionOutputs', 'UpdateRolesWithIDPFunctionRole'].includes(resource.LogicalResourceId));
    ...
    deleteResourceStack(envName) {
        ...
        const providerInfo = (_a = teamProviderInfo === null || teamProviderInfo === void 0 ? void 0 : teamProviderInfo[envName]) === null || _a === void 0 ? void 0 : _a[providerName];
        const stackName = providerInfo === null || providerInfo === void 0 ? void 0 : providerInfo.StackName;
        if (!stackName) {
            throw new AmplifyError('StackNotFoundError', {
