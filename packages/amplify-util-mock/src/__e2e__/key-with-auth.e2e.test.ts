...
beforeAll(async () => {
  ...
  const validSchema = /* GraphQL */ `
    ...
    type Post @auth(rules: [{ allow: owner }]) @model @key(name: "byAuthor", fields: ["author"]) {
      id: ID!
      title: String!
      owner: String
      author: String!
...
test('Test only authorized user can see results in custom queries generated by @key directive', async () => {
  await createPost(GRAPHQL_CLIENT_1, 'by author1', 'author1');
  await createPost(GRAPHQL_CLIENT_2, 'by author2', 'author2');

  const querResponse1 = await queryPostsByAuthor(GRAPHQL_CLIENT_1, 'author1');
  expect(querResponse1.data.queryPostsByAuthor).toBeDefined();
  expect(querResponse1.data.queryPostsByAuthor.items).toHaveLength(1);
  expect(querResponse1.data.queryPostsByAuthor.items[0].title).toEqual('by author1');
  expect(querResponse1.data.queryPostsByAuthor.items[0].author).toEqual('author1');

  const querResponse2 = await queryPostsByAuthor(GRAPHQL_CLIENT_1, 'author2');
  expect(querResponse2.data.queryPostsByAuthor).toBeDefined();
  expect(querResponse2.data.queryPostsByAuthor.items).toHaveLength(0);

  const querResponse3 = await queryPostsByAuthor(GRAPHQL_CLIENT_2, 'author2');
  expect(querResponse3.data.queryPostsByAuthor).toBeDefined();
  expect(querResponse3.data.queryPostsByAuthor.items).toHaveLength(1);
  expect(querResponse3.data.queryPostsByAuthor.items[0].title).toEqual('by author2');
  expect(querResponse3.data.queryPostsByAuthor.items[0].author).toEqual('author2');

  const querResponse4 = await queryPostsByAuthor(GRAPHQL_CLIENT_2, 'author1');
  expect(querResponse4.data.queryPostsByAuthor).toBeDefined();
  expect(querResponse4.data.queryPostsByAuthor.items).toHaveLength(0);
});

async function createPost(client: GraphQLClient, title: string, author: string) {
  const result = await client.query(
    /* GraphQL */ `
      mutation CreatePost($title: String!, $author: String!) {
        createPost(input: { author: $author, title: $title }) {
          createdAt
          id
          owner
          title
...
async function queryPostsByAuthor(client: GraphQLClient, author: string) {
  const result = await client.query(
    /* GraphQL */ `
      query queryPostByAuthor($author: String!) {
        queryPostsByAuthor(author: $author) {
          items {
            author
            createdAt
            id
