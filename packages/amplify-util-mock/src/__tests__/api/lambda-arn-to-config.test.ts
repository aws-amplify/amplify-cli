...
describe('lambda arn to config', () => {
  ...
  it('resolves string arns', async () => {
    const result = await lambdaArnToConfig(`aws::arn::something::region::lambda1::otherstuff`, {});
    expect(loadLambdaConfig_mock.mock.calls[0][0]).toEqual('lambda1');
    expect(result).toEqual(expectedLambdaConfig);
  });

  it('resolves Fn::Sub with params when lambda name is in template string', async () => {
    const result = await lambdaArnToConfig({ 'Fn::Sub': [`some::arn::lambda2::{withsubs}::stuff`, { withsubs: 'a value' }] }, {});
    expect(loadLambdaConfig_mock.mock.calls[0][0]).toEqual('lambda2');
    expect(result).toEqual(expectedLambdaConfig);
  });

  it('resolves Fn::Sub strings when lambda name is in template string', async () => {
    const result = await lambdaArnToConfig({ 'Fn::Sub': `some::{sub}::string::with::lambda1::name` }, {});
    expect(loadLambdaConfig_mock.mock.calls[0][0]).toEqual('lambda1');
    expect(result).toEqual(expectedLambdaConfig);
  });

  it('resolves Fn::GetAtt arns when lambda name is in resource name', async () => {
    const result = await lambdaArnToConfig({ 'Fn::GetAtt': [`functionlambda1resourcename`, 'arn'] }, {});
    expect(loadLambdaConfig_mock.mock.calls[0][0]).toEqual('lambda1');
    expect(result).toEqual(expectedLambdaConfig);
  });
