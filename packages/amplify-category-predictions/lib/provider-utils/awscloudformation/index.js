...
async function printRekognitionUploadUrl(context, resourceName, amplifyMeta, showOnAmplifyStatus) {
    ...
    if (parameters.adminTask) {
        ...
        const errMessage = 'Console command not supported for your configuration in the project. Use ‘amplify update predictions’ to modify your configurations.';
        context.print.error(errMessage);
...
async function openRekognitionUploadUrl(context, bucketName, region, folderPolicies, printOnlyURL) {
    const URL = folderPolicies === 'admin'
    ...
    context.print.info(`Rekognition endpoint to upload Images: ${chalk.blue.underline(URL)} (Amazon Rekognition only supports uploading PNG and JPEG files)`);
}
module.exports = {
    addResource,
    updateResource,
