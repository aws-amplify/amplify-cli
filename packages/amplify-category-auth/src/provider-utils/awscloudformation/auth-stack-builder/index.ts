export { AmplifyAuthCognitoStack } from './auth-cognito-stack-builder';
export { AmplifyAuthTransform } from './auth-stack-transform';
export { AmplifyUserPoolGroupStack } from './auth-user-pool-group-stack-builder';
export { AmplifyUserPoolGroupTransform } from './user-pool-group-stack-transform';
import { AmplifyStackTemplate } from 'amplify-cli-core';
import { AmplifyAuthCognitoStackTemplate, AmplifyUserPoolGroupStackTemplate } from './types';

export type AmpifyAuthTemplate = AmplifyAuthCognitoStackTemplate & AmplifyStackTemplate;
export type AmpifyUserPoolGroupTemplate = AmplifyUserPoolGroupStackTemplate & AmplifyStackTemplate;
