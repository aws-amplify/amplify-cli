...
export function getCreatedAtFieldName(directive: DirectiveNode): string | null {
  return getTimestampFieldName(directive, 'createdAt', 'createdAt');
}

export function getUpdatedAtFieldName(directive: DirectiveNode): string | null {
  return getTimestampFieldName(directive, 'updatedAt', 'updatedAt');
}

export function getTimestampFieldName(directive: DirectiveNode, fieldName: string, defaultFiledValue: string): string | null {
  const directiveArguments: ModelDirectiveArgs = getDirectiveArguments(directive);
  const timestamp: Record<string, any> = directiveArguments.timestamps as Record<string, any>;
