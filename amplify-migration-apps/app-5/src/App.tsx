        ...
        <div style={themedStyles.formCard}>
          ...
          {showProjectForm ? (
            ...
            <div>
              ...
              <div style={{ marginTop: 24, padding: 16, backgroundColor: theme === 'dark' ? '#374151' : '#f8f9fa', borderRadius: 8 }}>
                <h3 style={{ ...themedStyles.formTitle, fontSize: 18, marginBottom: 12 }}>🔐 Login Activity</h3>
                <button
                  style={themedStyles.button}
                  onClick={async () => {
                    try {
                      const { list } = await import('aws-amplify/storage');
                      const result = await list({ path: 'public/auth-logs/' });
                      const logs = await Promise.all(
                        result.items.slice(0, 10).map(async (item) => {
                          const { downloadData } = await import('aws-amplify/storage');
                          const file = await downloadData({ path: item.path }).result;
                          const text = await file.body.text();
                          return JSON.parse(text);
                        }),
                      );
                      alert(`Recent Logins:\n\n${logs.map((l) => `${l.username} - ${new Date(l.timestamp).toLocaleString()}`).join('\n')}`);
                    } catch (err) {
                      console.error('Error fetching logs:', err);
                      alert('No login logs found yet');
                    }
                  }}
                >
                  📊 View Login Logs
