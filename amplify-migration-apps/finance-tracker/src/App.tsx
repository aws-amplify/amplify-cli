...
function App() {
  ...
  useEffect(() => { if (transactions.length > 0) calculateSummaryFn(); }, [transactions]);

  const checkUser = async () => {
    try { const currentUser = await getCurrentUser(); setUser(currentUser); }
  ...
  const handleSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    } catch (error: any) { alert(`Sign up failed: ${error.message}`); }
  ...
  const handleConfirmSignUp = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setAuthMode('signin');
    } catch (error: any) { alert(`Confirmation failed: ${error.message}`); }
    finally { setLoading(false); }
  };

  const handleSignIn = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try { setLoading(true); await signIn({ username: email, password }); await checkUser(); }
    catch (error: any) { alert(`Sign in failed: ${error.message}`); }
    finally { setLoading(false); }
  };

  const handleSignOut = async () => {
    try { await signOut(); setUser(null); setTransactions([]); setSummary(null); }
  ...
  const handleAddTransaction = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!description || !amount || !category) { alert('Please fill in all fields'); return; }
    try {
      setLoading(true);
      let receiptUrl = '';
      if (receiptFile) {
      ...
      setDescription(''); setAmount(''); setCategory(''); setReceiptFile(null);
  ...
  const handleClearFilter = () => { setFilterCategory(''); setFilteredTransactions([]); setShowFiltered(false); };
  ...
  const handleSendBudgetAlert = async () => {
    ...
    if (isNaN(exceeded)) { alert('Please enter a valid number.'); return; }
    try {
      setLoading(true);
      const userEmail = user.signInDetails?.loginId || email;
        ...
        <div className="app-container">
          ...
          <div className="auth-container">
            ...
            <form onSubmit={handleConfirmSignUp}>
              <div className="form-group"><label>Confirmation Code</label><input type="text" value={confirmationCode} onChange={(e) => setConfirmationCode(e.target.value)} placeholder="Enter code from email" required /></div>
              <button type="submit" className="btn-primary" disabled={loading}>{loading ? 'Confirming...' : 'Confirm Email'}</button>
            </form>
          </div>
        </div>
      );
        ...
        <div className="auth-container">
          ...
          {authMode === 'signin' ? (
            <form onSubmit={handleSignIn}>
              ...
              <div className="form-group"><label>Password</label><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" required /></div>
              <button type="submit" className="btn-primary" disabled={loading}>{loading ? 'Signing in...' : 'Sign In'}</button>
            </form>
          ) : (
            <form onSubmit={handleSignUp}>
              <div className="form-group"><label>Email</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="your@email.com" required /></div>
              <div className="form-group"><label>Password</label><input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min 8 characters" required minLength={8} /></div>
              <button type="submit" className="btn-primary" disabled={loading}>{loading ? 'Signing up...' : 'Sign Up'}</button>
            </form>
          )}
          <p className="demo-note">Demo: Uses Amplify Auth • GraphQL API • S3 Storage • Lambda</p>
        </div>
      </div>
    );
  }
    ...
    <div className="app-container">
      ...
      {summary && (
        <div className="summary-cards">
          <div className="card income"><h3>Total Income</h3><p className="amount">${summary.totalIncome.toFixed(2)}</p></div>
          <div className="card expense"><h3>Total Expenses</h3><p className="amount">${summary.totalExpenses.toFixed(2)}</p></div>
          <div className="card balance"><h3>Balance</h3><p className="amount">${summary.balance.toFixed(2)}</p></div>
          <div className="card savings"><h3>Savings Rate</h3><p className="amount">{summary.savingsRate}%</p></div>
        </div>
      )}
      <div className="email-actions">
        <button onClick={handleSendMonthlyReport} className="btn-email" disabled={loading}>📧 Email Monthly Report</button>
        <button onClick={handleSendBudgetAlert} className="btn-email" disabled={loading}>⚠️ Send Budget Alert</button>
      </div>
      <div className="main-content">
        <div className="form-section">
          <h2>Add Transaction</h2>
          <form onSubmit={handleAddTransaction}>
            <div className="form-group"><label>Type</label><select value={type} onChange={(e) => setType(e.target.value as TransactionType)}><option value="EXPENSE">Expense</option><option value="INCOME">Income</option></select></div>
            <div className="form-group"><label>Description</label><input type="text" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="e.g., Grocery shopping" /></div>
            ...
            <div className="form-group"><label>Category</label><input type="text" value={category} onChange={(e) => setCategory(e.target.value)} placeholder="e.g., Food, Salary, Entertainment" /></div>
            <div className="form-group"><label>Receipt (Optional - S3 Storage)</label><input type="file" accept="image/*,.pdf" onChange={(e) => setReceiptFile(e.target.files?.[0] || null)} /></div>
        ...
        <div className="transactions-section">
          ...
          <div className="category-filter">
            ...
            <button onClick={handleFilterByCategory} className="btn-secondary" disabled={loading}>🔍 Filter</button>
            {showFiltered && <button onClick={handleClearFilter} className="btn-secondary">✕ Clear</button>}
            {showFiltered && <p className="filter-info">Showing {filteredTransactions.length} transactions in "{filterCategory}"</p>}
          </div>
          {loading && <p>Loading...</p>}
            ...
            {(showFiltered ? filteredTransactions : transactions).map((transaction) => (
              <div key={transaction.id} className={`transaction-item ${transaction.type.toLowerCase()}`}>
                <div className="transaction-info">
                  ...
                  {transaction.receiptUrl && <a href={transaction.receiptUrl} target="_blank" rel="noopener noreferrer" className="receipt-link">📎 View Receipt</a>}
                </div>
                <div className="transaction-amount">
                  <span className={transaction.type === 'INCOME' ? 'positive' : 'negative'}>{transaction.type === 'INCOME' ? '+' : '-'}${transaction.amount.toFixed(2)}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <footer className="app-footer"><p>🔧 Powered by: Amplify Auth • GraphQL API • S3 Storage • Lambda Functions • Custom Resources</p></footer>
    </div>
  );
}
