...
export function defineStoreLocatorSearch(backend: Backend) {
  ...
  const storeLocatorSearch = new geostoreLocatorSearch(storeLocatorSearchStack, 'storeLocatorSearch', {
  ...
  const policy = new Policy(storeLocatorSearch, 'gen1AuthPolicy', {
    statements: [
      new PolicyStatement({
        ...
        resources: [`arn:aws:geo:${storeLocatorSearchStack.region}:${storeLocatorSearchStack.account}:place-index/storeLocatorSearch-x`],
      }),
    ],
  });
