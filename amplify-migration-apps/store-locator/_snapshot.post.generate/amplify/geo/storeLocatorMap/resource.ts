...
export function defineStoreLocatorMap(backend: Backend) {
  ...
  const storeLocatorMap = new geostoreLocatorMap(storeLocatorMapStack, 'storeLocatorMap', {
  ...
  const policy = new Policy(storeLocatorMap, 'gen1AuthPolicy', {
    statements: [
      new PolicyStatement({
        ...
        resources: [`arn:aws:geo:${storeLocatorMapStack.region}:${storeLocatorMapStack.account}:map/storeLocatorMap-x`],
      }),
    ],
  });
