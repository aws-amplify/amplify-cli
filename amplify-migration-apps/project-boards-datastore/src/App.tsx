...
import { Project, Todo, ProjectStatus } from './models';
import type { CreateProjectInput, CreateTodoInput, UpdateTodoInput } from './API';
...
const TodoCard: React.FC<{
  ...
  return (
    <div style={cardStyle}>
      ...
      {/* Project Selector */}
      {user?.username && projects.length > 0 && (
        <div style={{ marginTop: 8 }}>
          <select
...
const AuthenticatedApp: React.FC<AppProps> = ({ signOut, user }) => {
  ...
  async function handleTodoDelete(todoId: string) {
    try {
      await DataStore.delete(Todo, todoId);
