...
export function createTestOrchestrator(
  testFunctions: ReturnType<typeof createTestFunctions>,
  runTest: ReturnType<typeof createTestRunner>['runTest'],
) {
  async function runPublicQueryTests(): Promise<void> {
    ...
    await runTest('getRandomQuote', testFunctions.testGetRandomQuote);
    const projectId = await runTest('listProjects', testFunctions.testListProjects);
    const todoId = await runTest('listTodos', testFunctions.testListTodos);

    if (projectId) await runTest('getProject', () => testFunctions.testGetProject(projectId));
    if (todoId) await runTest('getTodo', () => testFunctions.testGetTodo(todoId));
  }
  ...
  async function runMutationTests(): Promise<void> {
    ...
    const projectId = await runTest('createProject', testFunctions.testCreateProject);
    const todoId = await runTest('createTodo', () => testFunctions.testCreateTodo(projectId || undefined));

    // Update project and todo
    if (projectId) await runTest('updateProject', () => testFunctions.testUpdateProject(projectId));
    if (todoId) await runTest('updateTodo', () => testFunctions.testUpdateTodo(todoId, projectId || undefined));

    // Cleanup: delete todo and project
    if (todoId) await runTest('deleteTodo', () => testFunctions.testDeleteTodo(todoId));
    if (projectId) await runTest('deleteProject', () => testFunctions.testDeleteProject(projectId));
  }
  ...
  async function runStorageTests(): Promise<void> {
    ...
    const uploadedPath = await runTest('uploadImage', testFunctions.testUploadImage);

    if (uploadedPath) {
      await runTest('getUrl', () => testFunctions.testGetUrl(uploadedPath));
      await runTest('getProperties', () => testFunctions.testGetProperties(uploadedPath));
      await runTest('downloadData', () => testFunctions.testDownloadData(uploadedPath));

      // Create a todo with the uploaded image
      console.log('\n📝 Creating todo with uploaded image...');
      await runTest('createTodoWithImage', () => testFunctions.testCreateTodo(undefined, [uploadedPath]));
