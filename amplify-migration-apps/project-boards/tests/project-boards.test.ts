...
describe('getRandomQuote', () => {
  it('returns a quote with all expected fields', async () => {
    ...
    const result = await client.graphql({ query: getRandomQuote });
...
describe('S3 Storage', () => {
  ...
  it('uploads a file via uploadData', async () => {
  ...
  it('gets a signed URL via getUrl', async () => {
  ...
  it('gets file properties via getProperties', async () => {
    const properties = await getProperties({ path: uploadedPath });
  ...
  it('downloads a file via downloadData', async () => {
    const downloadResult = await downloadData({ path: uploadedPath }).result;
