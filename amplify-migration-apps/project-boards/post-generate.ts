...
async function updateBranchName(appPath: string): Promise<void> {
  ...
  if (branchNameMatch) {
    ...
    console.log('  WARNING: No branchName property found in migratedAmplifyGen1DynamoDbTableMappings');
  }
  ...
  // Verify the update
  const verifyContent = await fs.readFile(resourcePath, 'utf-8');
  const verifyMatch = verifyContent.match(/branchName:\s*['"]([^'"]+)['"]/);
  if (verifyMatch) {
    console.log(`  Verified branchName is now: '${verifyMatch[1]}'`);
  }
}
