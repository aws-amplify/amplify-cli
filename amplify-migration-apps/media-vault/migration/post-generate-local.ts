...
/**
 * Post-generate script for media-vault app (LOCAL variant).
 *
 * Same as post-generate.ts, except step 7 (monkeyPatchAuthSecret) is
 * commented out. Use this when you plan to deploy from your own machine
 * (e.g. `npx ampx sandbox` or `npx ampx pipeline-deploy`) with real
 * secrets already configured in Parameter Store / Secrets Manager —
 * the monkey-patch replaces `secret()` with plaintext stubs that are
 * only appropriate for the CLI's automated e2e harness.
 *
