...
export class CfnIncludeGenerator {
  async generateWrapper(resourceName: string, className: string, dependencies: string[], outputPath: string): Promise<void> {
    const constructorParams = dependencies.map((cat) => `${AmplifyHelperTransformer.CATEGORY_MAP[cat] || cat}?: any`).join(', ');
