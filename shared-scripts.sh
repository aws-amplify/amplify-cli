...
# Run Gen2 Migration E2E test from source (no pkg binary)
# This is slower but avoids pkg bundling issues
function _runGen2MigrationE2EFromSource {
    echo "Running Gen2 Migration E2E test from source for app: $MIGRATION_APP"

    # Build from source
    yarn install
    yarn build

    # Add the CLI bin to PATH and verify
    export PATH=$CODEBUILD_SRC_DIR/packages/amplify-cli/bin:$PATH
    amplify version
