...
function generatePkgCli {
  ...
  if [[ "$@" =~ 'arm' ]]; then
    npx pkg --no-bytecode --public-packages "*" --public -t linux-arm64 ../build/node_modules -o ../out/amplify-pkg-linux-arm64
    tar -czvf ../out/amplify-pkg-linux-arm64.tgz ../out/amplify-pkg-linux-arm64
  fi

  if [[ "$@" =~ 'linux' ]]; then
    npx pkg -t linux-x64 ../build/node_modules -o ../out/amplify-pkg-linux-x64
    tar -czvf ../out/amplify-pkg-linux-x64.tgz ../out/amplify-pkg-linux-x64
  fi

  if [[ "$@" =~ 'macos' ]]; then
    npx pkg -t macos-x64 ../build/node_modules -o ../out/amplify-pkg-macos-x64
    tar -czvf ../out/amplify-pkg-macos-x64.tgz ../out/amplify-pkg-macos-x64
  fi

  if [[ "$@" =~ 'win' ]]; then
    npx pkg -t win-x64 ../build/node_modules -o ../out/amplify-pkg-win-x64.exe
